...
describe.skipIf(!hasDb)(
  "Platform backoffice W4 e2e (fleet · kill switch · flags · impersonation)",
  () => {
    ...
    beforeAll(async () => {
      ...
      tenantToken = await signDevToken(SECRET, {
        sub: `auth|bo-w4-${suffix}`,
        email: tenantUser.email,
      });

      const staffRow = await owner.platformStaff.create({
        data: {
          email: `bo-w4-ops-${suffix}@cf.test`,
          name: "Ops",
          role: "OPERATOR",
          status: "ACTIVE",
        },
      ...
      staffId = staffRow.id;
      staffToken = await signStaffToken({
        sub: staffRow.id,
    ...
    it("kill the email channel → test-send refuses CHANNEL_KILLED (422) → clear restores", async () => {
      ...
      const row = list.body.find(
        (r: { agencyId: string; channel: string }) =>
          r.agencyId === agencyId && r.channel === "email",
      );
    ...
    it("a per-tenant feature flag toggles and is audited", async () => {
      ...
      const list = await request(app.getHttpServer())
        .get(`/backoffice/workspaces/${wsA}/flags`)
        .set(staff());
      ...
      await request(app.getHttpServer())
        ...
        .expect(201);
      const after = await owner.featureFlag.findMany({
        where: { workspaceId: wsA, key: "new_editor" },
      });
    ...
    it("fleet health CONSUMES P5-W1's shared computation + surfaces outliers", async () => {
      ...
      const scored = res.body.health.scores.find(
        (s: { senderId: string }) => s.senderId === senderId,
      );
      ...
      const outlier = res.body.outliers.find(
        (o: { workspaceId: string; metric: string }) =>
          o.workspaceId === wsA && o.metric === "bounces",
      );
    ...
    it("version pins are read-only platform-scope visibility", async () => {
      ...
      expect(
        res.body.prompts.every((p: { version: number }) => typeof p.version === "number"),
      ).toBe(true);
    });
