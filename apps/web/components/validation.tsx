...
> = {
  ...
  risky: {
    ...
    fg: "#9A6B12",
    title: "Risky address — held from sending (workspace policy)",
  },
  invalid: {
    label: "Invalid",
    bg: "rgba(224,121,107,.14)",
    fg: "#C9543F",
    title: "Invalid address — excluded from campaigns",
  },
  unverified: {
    label: "Unverified",
    bg: "#F2EEE4",
    fg: "#8A7F6B",
    title: "Validation pending — held until verified",
  },
};
...
export function ValidationReportCard({
  ...
  return (
    <div
      ...
      data-testid="csv-validation-card"
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
        ...
        {c && c.pending > 0 ? (
          <span
            ...
            data-testid="csv-validation-pending"
          >
            Validating…
          </span>
      ...
      {!report ? (
        ...
        <>
          {c && c.pending > 0 ? (
            <div
              style={{ fontSize: 12.5, color: "#5C6B62", marginBottom: 6 }}
              data-testid="csv-validation-progress"
            >
              Validating {c.pending.toLocaleString()} contact{c.pending === 1 ? "" : "s"} — sending
              starts as they clear.
            </div>
          ) : null}
          {report.status === "held" && report.heldReason ? (
            <div
              style={{ fontSize: 12.5, color: "#9A6B12", marginBottom: 6 }}
              data-testid="csv-validation-held"
            >
              {VALIDATION_HELD_COPY[report.heldReason] ??
                "Validation queued — contacts stay safely held until verified."}
            </div>
          ) : null}
          {resolvedAny && c ? (
            <div
              style={{ fontSize: 13, fontWeight: 600, color: "#0E1512" }}
              data-testid="csv-validation-counts"
            >
              {c.valid.toLocaleString()} valid · {c.risky.toLocaleString()} risky (held) ·{" "}
              {c.invalid.toLocaleString()} invalid (excluded) ·{" "}
              {c.skippedSuppressed.toLocaleString()} already suppressed
            </div>
          ) : null}
          {invalidRows && invalidRows.length > 0 ? (
            <div
              ...
              data-testid="csv-validation-invalid-rows"
            >
              {invalidRows.map((r, i) => (
                <div
                  key={r.contactId}
                ...
                >
                  ...
                  <span style={{ fontSize: 11.5, color: "#C9543F", minWidth: 0 }}>
                    {r.detail ?? r.via ?? "invalid"}
                  </span>
                </div>
              ))}
            </div>
          ...
          {c && c.invalid + c.skippedSuppressed > 0 ? (
            <a
              href={`/api/cf/contacts/validation-batches/${batchId}/exclusions.csv`}
              download
              style={{
              ...
              data-testid="csv-validation-download"
            >
