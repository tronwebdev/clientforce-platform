...
export const humanizeHeader = (h: string) =>
  h.replace(/_/g, " ").replace(/^./, (ch) => ch.toUpperCase());
...
const CSV_FIELD_KEY: Record<string, string> = {
...
export function ContactImportFlow({
  ...
  useEffect(() => {
    ...
    let dead = false;
    const poll = async () => {
      const report = (await cf(`contacts/validation-batches/${valBatchId}`).catch(
        () => null,
      )) as ValidationBatchReport | null;
      if (dead || !report) return;
      setValReport(report);
      if (report.counts.pending === 0 && report.counts.invalid > 0) {
        const detail = (await cf(
          `contacts/validation-batches/${valBatchId}/rows?outcome=invalid&take=8`,
        ).catch(() => null)) as { rows: ValidationBatchRow[] } | null;
  ...
  function snapReview() {
    ...
    const unsubEmails = new Set(
      (existingRows ?? []).filter((r) => r.unsub).map((r) => (r.email ?? "").toLowerCase()),
    );
  ...
  async function runImport() {
      ...
      if (m.startsWith("custom:")) customKeyByCol.set(i, m.slice(7));
        ...
        try {
          const def = (await cf("contact-fields", {
            method: "POST",
            body: JSON.stringify({
              label: humanizeHeader(csvFile.headers[i] ?? ""),
              origin: "csv_import",
            }),
    ...
    for (const r of reviewSnap.valid) {
      ...
      if (!parsed.success)
        prefailed.push({
          index: -1,
          email: String(payload.email ?? "(no email)"),
          reason: "Invalid email address — not imported",
        });
  ...
  async function executeImport(
      ...
      try {
        ...
        chunk.forEach((row, i) =>
          agg.failed.push({
            index: start + i,
            email: row.email,
            reason: "Network error — row not imported",
          }),
        );
      }
      setImportProg({
        done: Math.min(start + chunk.length, rowsToSend.length),
    ...
    if (bgCloseRef.current) {
      ...
      toast(
        `Imported ${agg.created} contact${agg.created === 1 ? "" : "s"}${agg.skippedDuplicates ? ` · ${agg.skippedDuplicates} duplicate${agg.skippedDuplicates === 1 ? "" : "s"} skipped` : ""}${fails ? ` · ${fails} failed` : ""}`,
      );
      setImportProg(null);
  ...
  async function retryFailed() {
    ...
    const retryRows = importResult.failed
      .filter((f) => f.index >= 0)
      .map((f) => sentRowsRef.current[f.index])
      .filter((r): r is ImportContactRow => Boolean(r));
        ...
        {csvStep < 3 ? (
          <>
            <div style={{ padding: "16px 22px 0" }}>
                ...
                {[0, 1, 2].map((n) => (
                  <span
                    ...
                    style={{
                      ...
                      height: 5,
                      borderRadius: 100,
                      background: n <= csvStep ? "#16A82A" : "#E4EAE6",
                    }}
                  />
                ))}
              </div>
              ...
              >
                Step {Math.min(csvStep + 1, 3)} of 3
              </div>
            </div>
            <div style={{ padding: "16px 22px 20px" }}>
              {csvStep === 0 ? (
                <>
                  <div
                    style={{ fontSize: 16, fontWeight: 700, color: "#0E1512", marginBottom: 14 }}
                  >
                    Upload your file
                  </div>
                  <label
                  ...
                  >
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (f) void f.text().then((t) => loadCsv(f.name, t));
                        e.target.value = "";
                      }}
                    />
                    <div style={{ fontSize: 28, marginBottom: 9 }}>📄</div>
                    <div
                      style={{ fontSize: 14.5, fontWeight: 700, color: "#0E1512", marginBottom: 3 }}
                    >
                      Drop your CSV here or browse
                    </div>
                    <div style={{ fontSize: 12.5, color: "#9AA59E" }}>
                      .csv up to 50 MB · first row should be column headers
                    </div>
                  </label>
                  {csvFile ? (
                    <div
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 11.5, color: "#9AA59E" }}>
                          {csvFile.rows.length} rows · {csvFile.headers.length} columns detected
                        </div>
                      </div>
                      <span
                        onClick={() => {
                        ...
                        style={{ color: "#9AA59E", cursor: "pointer" }}
                      >
                        ✕
                      </span>
                    </div>
              ...
              {csvStep === 1 && csvFile ? (
                <>
                  ...
                  <div style={{ fontSize: 13, color: "#9AA59E", marginBottom: 14 }}>
                    We matched these automatically — adjust any that look off.
                  </div>
                  <div
                    style={{
                      display: "grid",
                      gridTemplateColumns: "1fr 22px 1.1fr",
                      gap: 8,
                      paddingBottom: 7,
                      borderBottom: "1px solid #EBE3D6",
                      marginBottom: 4,
                  ...
                  {csvFile.headers.map((h, i) => {
                    ...
                    const display = isNew
                      ? `＋ ${title} · new field`
                      : picked.startsWith("custom:")
                        ? (fieldDefs.find((d) => d.key === picked.slice(7))?.label ??
                          picked.slice(7))
                        : picked;
                    const pick = (v: string) => {
                      setCsvMap((m) => m.map((x, j) => (j === i ? v : x)));
                      setMapDD(null);
                    };
                    return (
                      <div
                      ...
                      >
                        ...
                        <span style={{ color: "#C2B79F", textAlign: "center", fontSize: 12 }}>
                          →
                        </span>
                        <div style={{ position: "relative" }}>
                          <div
                            onClick={() => setMapDD((v) => (v === i ? null : i))}
                            style={{
                              border: `1px solid ${isNew ? "#9AD6E4" : "#EBE3D6"}`,
                              borderRadius: 9,
                              padding: "8px 11px",
                              fontSize: 12.5,
                              fontWeight: 600,
                              color: isSkip ? "#9AA59E" : isNew ? "#1192A6" : "#0E1512",
                              background: "#FBF7F0",
                              display: "flex",
                              alignItems: "center",
                              cursor: "pointer",
                            }}
                            data-testid={`csv-map-${i}`}
                          >
                            <span
                              style={{
                                overflow: "hidden",
                            ...
                            <span style={{ marginLeft: "auto", color: "#9AA59E", paddingLeft: 6 }}>
                              ⌄
                            </span>
                          </div>
                          {mapDD === i ? (
                            ...
                            >
                              <div style={{ maxHeight: 196, overflowY: "auto" }}>
                                ...
                                {CSV_FIELDS.filter((f) => f !== "Skip this column").map((f) => (
                                  ...
                                  >
                                    ...
                                    <span
                                      style={{
                                        color: "#16A82A",
                                        visibility: picked === f ? "visible" : "hidden",
                                      }}
                                    >
                                      ✓
                                    </span>
                                ...
                                {fieldDefs.map((d) => (
                                  <div
                                    ...
                                    data-testid={`csv-map-custom-${d.key}`}
                                  >
                                    ...
                                    <span
                                      style={{
                                        color: "#16A82A",
                                        visibility:
                                          picked === `custom:${d.key}` ? "visible" : "hidden",
                                      }}
                                    >
                                      ✓
                                    </span>
                              ...
                              >
                                ...
                                <span
                                  style={{
                                    color: "#16A82A",
                                    visibility: isSkip ? "visible" : "hidden",
                                  }}
                                >
                                  ✓
                                </span>
              ...
              {csvStep === 2 && reviewSnap ? (
                <>
                  ...
                  <div style={{ fontSize: 13, color: "#9AA59E", marginBottom: 16 }}>
                    Here&apos;s what we&apos;ll add to your contacts.
                  </div>
                  ...
                  <div style={{ marginBottom: 14, position: "relative" }}>
                    ...
                    <div
                      ...
                      data-testid="csv-list"
                    >
                      <span
                        style={{
                          color: csvListId ? "#0E1512" : ensureDefaultList ? "#16A82A" : "#9AA59E",
                          fontWeight: csvListId || !ensureDefaultList ? undefined : 600,
                        }}
                      >
                        {csvListId
                          ? (lists.find((l) => l.id === csvListId)?.name ??
                            ensuredListRef.current?.name ??
                      ...
                      >
                        ...
                        <div
                          ...
                          style={{
                            padding: "9px 14px",
                            fontSize: 13.5,
                            color: ensureDefaultList ? "#16A82A" : "#5C6B62",
                            fontWeight: ensureDefaultList ? 600 : undefined,
                            cursor: "pointer",
                          }}
                          data-testid="csv-list-default"
                        >
                          {ensureDefaultList
                            ? `＋ New list “${(csvFile?.name ?? "import").replace(/\.[^.]+$/, "")}”`
                            : "No list (all contacts)"}
                        ...
                        {lists.map((l) => (
                          <div
                            ...
                            style={{
                              ...
                              color: "#0E1512",
                              cursor: "pointer",
                              background: csvListId === l.id ? "rgba(53,232,52,.07)" : "#fff",
                            }}
                            data-testid={`csv-list-opt-${l.id}`}
                          >
                            <span
                              style={{
                                width: 24,
                  ...
                  >
                    {[
                      ...
                      {
                        value: String(reviewSnap.dupes),
                        label: "Duplicates skipped",
                        fg: "#1192A6",
                      },
                      {
                        value: String(reviewSnap.suppressed),
                  ...
                  {reviewSnap.createCount > 0 ? (
                    <div
                      ...
                      data-testid="csv-create-note"
                    >
                      <span style={{ color: "#1192A6" }}>＋</span>
                      <span style={{ fontSize: 12.5, color: "#1192A6", fontWeight: 600 }}>
                        {reviewSnap.createCount} new custom field
                        {reviewSnap.createCount === 1 ? "" : "s"} will be created:{" "}
                        {csvFile!.headers
                          .filter((_, i) => csvMap[i] === CSV_CREATE)
                          .map(humanizeHeader)
                          .join(", ")}
                      </span>
                    </div>
                  ...
                  {csvError ? (
                    <div
                      style={{
                        ...
                        border: "1px solid rgba(224,121,107,.3)",
                        borderRadius: 11,
                        padding: "11px 14px",
                        marginBottom: 10,
                      }}
                    ...
                    >
                      ...
                      <span style={{ fontSize: 12.5, color: "#C9543F", fontWeight: 600 }}>
                        {csvError}
                      </span>
                    </div>
                  ) : null}
                  <div
                    style={{
                      ...
                      border: "1px solid rgba(53,232,52,.22)",
                      borderRadius: 11,
                      padding: "11px 14px",
                    }}
                  >
                    <span style={{ color: "#16A82A" }}>✓</span>
                    <span style={{ fontSize: 12.5, color: "#16A82A", fontWeight: 600 }}>
                      All contacts checked against your suppression list.
                    </span>
                  </div>
                  ...
                  {importing && importProg ? (
                    <div style={{ marginTop: 12 }} data-testid="import-progress">
                      ...
                      >
                        <span style={{ fontSize: 13, fontWeight: 700, color: "#0E1512" }}>
                          Importing… {importProg.done} of {importProg.total}
                        </span>
                        <span style={{ fontSize: 12, fontWeight: 600, color: "#8A7F6B" }}>
                          {importProg.total > 0
                            ? Math.round((importProg.done / importProg.total) * 100)
                            : 0}
                          %
                        </span>
                      </div>
                      ...
                      >
                        <div
                          style={{
                            height: "100%",
                            width: `${importProg.total > 0 ? (importProg.done / importProg.total) * 100 : 0}%`,
                            borderRadius: 100,
                            background: GRAD,
                      ...
                      <div style={{ fontSize: 11.5, color: "#9AA59E", marginTop: 7 }}>
                        You can close this window — the import keeps running and we&apos;ll confirm
                        when it&apos;s done.
                      </div>
                    </div>
                  ) : null}
              ...
              {(() => {
                ...
                const canGo =
                  ...
                  (csvStep === 0
                    ? Boolean(csvFile)
                    : csvStep === 1
                      ? csvMap.includes("Email")
                      : (reviewSnap?.newCount ?? 0) > 0);
                const label = importing
                  ? "Importing…"
                  : csvStep === 2
                    ? `Import ${reviewSnap?.newCount ?? 0} contact${(reviewSnap?.newCount ?? 0) === 1 ? "" : "s"}`
                    : "Continue";
                return (
                  <span
                    onClick={() => {
                      if (!canGo) return;
                      if (csvStep === 2) void runImport();
                      else {
                        if (csvStep === 1) snapReview();
                        setCsvStep((v) => v + 1);
                      }
                    }}
                    style={{
                      ...
                      fontSize: 14,
                      fontWeight: 700,
                      color: canGo ? "#0A0F0C" : "#9AA59E",
                      background: canGo ? GRAD : "#ECE7DC",
                      borderRadius: 11,
                      padding: "10px 22px",
                      cursor: canGo ? "pointer" : "not-allowed",
                      boxShadow: canGo ? "0 6px 16px rgba(53,232,52,.26)" : "none",
                    }}
          ...
          (() => {
            ...
            const tiles = [
              ...
              {
                value: String(res.failed.length),
                label: "Failed",
                fg: hasFails ? "#C9543F" : "#0E1512",
              },
            ];
            return (
              <>
                <div
                  style={{ padding: "30px 28px 22px", textAlign: "center" }}
                  data-testid={hasFails ? "csv-error-summary" : "csv-done"}
                >
                  <div
                    style={{
                      width: 60,
                      height: 60,
                      borderRadius: "50%",
                      background: hasFails ? "rgba(224,121,107,.14)" : "#D7F5DD",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      fontSize: 28,
                      color: hasFails ? "#C9543F" : "#16A82A",
                      margin: "0 auto 18px",
                    }}
                  >
                    {hasFails ? "⚠" : "✓"}
                  ...
                  >
                    {hasFails
                      ? `${res.created} imported · ${res.failed.length} failed`
                      : `${res.created} contact${res.created === 1 ? "" : "s"} imported`}
                  </div>
                  <div
                    style={{
                      fontSize: 13.5,
                      ...
                      {res.failed.map((f, i) => (
                        <div
                          ...
                          style={{
                            ...
                            gap: 10,
                            padding: "9px 13px",
                            borderTop: i === 0 ? "none" : "1px solid #F3EEE4",
                          }}
                        >
                          <span
                            style={{
                          ...
                          <span style={{ fontSize: 12, color: "#C9543F", minWidth: 0 }}>
                            {f.reason}
                          </span>
                        </div>
                      ))}
                ...
                >
                  ...
                  {hasFails && retryable > 0 ? (
                    <span
                      onClick={() => {
                        if (!importing) void retryFailed();
                      }}
                      style={{
                        marginLeft: "auto",
                        fontSize: 14,
                        fontWeight: 700,
                        color: importing ? "#9AA59E" : "#0A0F0C",
                        background: importing ? "#ECE7DC" : GRAD,
                        borderRadius: 11,
                        padding: "10px 24px",
                        cursor: importing ? "not-allowed" : "pointer",
                        boxShadow: importing ? "none" : "0 6px 16px rgba(53,232,52,.26)",
                      }}
                      data-testid="csv-retry-failed"
                    >
                      {importing && importProg
                        ? `Retrying… ${importProg.done} of ${importProg.total}`
