...
export type BoldDrawerState =
  | {
      ...
      read: string;
      breakLabel: string;
      rows: Array<{ n: string; v: string; w: number; c: string }>;
    }
  | {
      t: "grp";
      agentId: string;
...
const GRP_ORDER: Record<string, number> = {
  replied: 0,
  booked: 1,
  opened: 2,
  delivered: 3,
...
function timelineLine(type: string, payload: unknown): string {
  ...
  if (type.endsWith(".replied.v1"))
    return `Replied${typeof p.intent === "string" ? ` — ${p.intent.replace(/_/g, " ")}` : ""}.`;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {state.t === "person" ? (
          <PersonBody state={state} onClose={onClose} flash={flash} onMessage={onMessage} />
        ) : null}
      </div>
    </div>
  );
...
function NumBody({
  ...
  state: Extract<BoldDrawerState, { t: "num" }>;
  onClose: () => void;
}) {
  return (
    <>
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div
            style={{ ...mono, fontSize: 9.5, letterSpacing: ".18em", color: "var(--cvb-faint)" }}
          >
            {state.label}
          </div>
          <div
      ...
      <div style={{ fontSize: 13.5, color: "var(--cvb-muted)", lineHeight: 1.6, marginTop: 14 }}>
        {state.read}
      </div>
...
function GrpBody({
  ...
  state: Extract<BoldDrawerState, { t: "grp" }>;
  onClose: () => void;
}) {
  ...
  if (sort === "status")
    list.sort((a, b) => (GRP_ORDER[a.status] ?? 9) - (GRP_ORDER[b.status] ?? 9));
  else if (sort === "name")
    list.sort((a, b) => contactName(a.contact).localeCompare(contactName(b.contact)));
  ...
  return (
    <>
      ...
      {list.map((r) => {
        ...
        return (
          <div
            key={r.contact.id + r.sentAt}
            style={{
              display: "flex",
              alignItems: "center",
              gap: 11,
              padding: "11px 2px",
              borderBottom: "1px solid var(--cvb-line-2)",
            }}
          >
            <span
              style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "-.018em" }}>
                {contactName(r.contact)}
              </div>
              <div
                style={{ fontSize: 11, color: "var(--cvb-faint)", marginTop: 2, lineHeight: 1.4 }}
              >
      ...
      {recipients != null && list.length === 0 ? (
        <div style={{ fontSize: 12.5, color: "var(--cvb-faint)", padding: "20px 2px" }}>
...
function PersonBody({
  ...
  const [browserCall, setBrowserCall] = useState<{
    callId: string;
    sandbox: boolean;
    token?: string;
  } | null>(null);
  ...
  async function addToList(list: ContactListDto) {
    ...
    flash?.(
      added === 0 ? `Already in “${list.name}” — nothing to add.` : `Added to “${list.name}”.`,
    );
  }
  ...
  async function setCallConsent(value: "granted" | "denied" | "unknown") {
    ...
    flash?.(
      value === "granted"
        ? "Ada may call them now."
        : value === "denied"
          ? "Ada will not call them."
          : "Call permission cleared.",
    );
  }
  ...
  const metaLine = [row?.company, state.contact.email ?? row?.email, row?.phone]
    .filter(Boolean)
    .join(" · ");
    ...
    <>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0, paddingTop: 3 }}>
          <div
            className="cvb-display"
            style={{ fontWeight: 900, fontSize: 20, letterSpacing: "-.03em", lineHeight: 1.15 }}
            data-testid="bold-person-name"
          >
            {contactName(state.contact)}
          </div>
          <div style={{ fontSize: 12, color: "var(--cvb-faint)", marginTop: 4 }}>
            {metaLine || "No email on record"}
          </div>
          {st ? (
            <span
              style={{
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 18, flexWrap: "wrap" }}>
          ...
          {listOpen ? (
            ...
            >
              {lists.map((l) => (
                ...
                >
                  {l.name}
                  <span style={{ ...mono, fontSize: 10, color: "var(--cvb-faint)", marginLeft: 6 }}>
                    {l.memberCount}
                  </span>
                </span>
              ))}
        ...
        <span
          ...
          style={{
            ...
            fontWeight: 700,
            color: "var(--cvb-slate)",
            background: "var(--cvb-slate-tint)",
            border: "1px solid var(--cvb-slate-line)",
            borderRadius: 11,
            padding: "9px 13px",
            cursor: "pointer",
          }}
        ...
        >
          ◷ Book
          <span style={{ fontSize: 10, fontWeight: 600, color: "var(--cvb-ghost)" }}>
            Coming soon
          </span>
        </span>
      </div>
      ...
      {callOpen ? (
        <div
          data-testid="bold-person-callsheet"
          style={{
            marginTop: 12,
        ...
        >
          {!callAgentId ? (
            ...
            <div
              data-testid="bold-person-call-blocked"
              style={{ fontSize: 12.5, color: "var(--cvb-amber)", lineHeight: 1.5 }}
            >
              {row
              ...
              {callAgentId ? (
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 10 }}>
                  <span
                    style={{ fontSize: 12, color: "var(--cvb-muted)", flex: 1, lineHeight: 1.45 }}
                  >
                  ...
                  <span
                    ...
                    style={{
                      ...
                      fontWeight: 800,
                      color: "var(--cvb-card)",
                      background: asking ? "var(--cvb-ghost)" : "var(--cvb-forest)",
                      borderRadius: 10,
                      padding: "7px 12px",
                      cursor: "pointer",
                      flex: "none",
            ...
            <>
              <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "-.016em" }}>
                Ada picks the best time
              </div>
              <div
                data-testid="bold-person-call-window"
                style={{
                  ...mono,
                  fontSize: 10,
                  color: "var(--cvb-muted)",
              ...
              >
                {callWindow
                  ? `${callWindow.window.start < callWindow.window.floorStart ? callWindow.window.floorStart : callWindow.window.start}–${
                      callWindow.window.end > callWindow.window.floorEnd
                        ? callWindow.window.floorEnd
                        : callWindow.window.end
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12 }}>
                ...
                <span
                  ...
                  style={{
                    ...
                    fontWeight: 800,
                    color: "var(--cvb-card)",
                    background: callWindow ? "var(--cvb-forest)" : "var(--cvb-ghost)",
                    borderRadius: 10,
                    padding: "8px 13px",
                    cursor: callWindow ? "pointer" : "default",
                    flex: "none",
          ...
          {callAgentId ? (
            <div
              style={{
                marginTop: 12,
                paddingTop: 12,
                borderTop: "1px solid var(--cvb-line-ctl)",
                display: "flex",
                alignItems: "center",
                gap: 10,
              }}
            >
              ...
              <span
                ...
                style={{
                  ...
                  cursor: "pointer",
                  flex: "none",
                  opacity: humanDialing || browserCall ? 0.6 : 1,
                }}
              >
            ...
            return (
              <span
                ...
                style={{
                  ...
                  background: on ? tone[1] : "transparent",
      ...
      {nextStep ? (
        <div
          data-testid="bold-person-nextstep"
          style={{
            marginTop: 14,
            background: "var(--cvb-panel)",
            border: `1px ${nextStep.live ? "solid" : "dashed"} var(--cvb-line-ctl)`,
            borderRadius: 13,
            padding: "11px 14px",
            display: "flex",
            alignItems: "center",
        ...
        >
          ...
          <span
            data-testid="bold-person-nextstep-why"
            style={{ ...mono, fontSize: 10, color: "var(--cvb-muted)", flex: 1, minWidth: 90 }}
          >
            {nextStep.provenance}
          </span>
          {nextStep.live ? (
            ...
            <span
              data-testid="bold-person-nextstep-deferred"
              style={{ fontSize: 11.5, fontWeight: 700, color: "var(--cvb-faint)", flex: "none" }}
            >
        ...
        <>
          ...
          {enrollments.map((e) => (
            <div
              ...
              data-testid={`bold-person-camp-${e.id}`}
            >
              <span
                style={{
                  width: 6,
                  ...
                  borderRadius: "50%",
                  flex: "none",
                  background: e.status === "ACTIVE" ? "var(--cvb-forest)" : "var(--cvb-ghost)",
                }}
              />
              <span
                style={{
      ...
      {row ? (
        <>
          ...
          {[
            ...
            ...Object.entries(row.custom).map(
              ([k, v]) => [k, v == null ? null : String(v)] as [string, string | null],
            ),
          ]
            ...
            .map(([k, v]) => (
              <div
                key={k}
                style={{ display: "flex", alignItems: "baseline", gap: 10, padding: "5px 0" }}
              >
                <span
                  style={{
                    ...mono,
          ...
          {row.valueEstCents != null &&
          row.goal &&
          (row.stage === "booked" || row.stage === "won") ? (
      ...
      {timeline == null ? (
        <div style={{ fontSize: 12.5, color: "var(--cvb-faint)" }}>Loading…</div>
      ) : null}
      {timeline?.map((e, i) => {
            ...
            >
              ...
              <span
                style={{
                  ...
                  flex: 1,
                  background: "var(--cvb-line-inner)",
                  minHeight: i === timeline.length - 1 ? 0 : 18,
                }}
              />
            </div>
            <div style={{ flex: 1, minWidth: 0, paddingBottom: 18 }}>
              <div style={{ fontSize: 13, color: "var(--cvb-ink)", lineHeight: 1.5 }}>
                {timelineLine(e.type, e.payload)}
              </div>
      ...
      {(() => {
        ...
        return (
          ...
          >
            ...
            <span style={{ fontSize: 12.5, color: "var(--cvb-forest)", lineHeight: 1.5 }}>
              {line}
            </span>
          </div>
        );
