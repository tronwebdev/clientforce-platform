    ...
    return (
      <div style={{ padding: "26px 40px 40px" }} data-testid="bold-proposals">
        <BoldSegRow
          ...
          active="Yours"
          onPick={() => {}}
          cta="Ask Ada to draft"
          onCta={onBuild}
          ctaTestId="bold-proposals-build"
        />
        {rows && rows.length === 0 ? (
          <div
            style={{
  ...
  return (
    <div data-testid="bold-proposal-detail">
      ...
      >
        <span
          onClick={back}
          data-testid="bold-proposal-back"
        ...
        <span style={{ flex: 1 }} />
        <span
          title="Coming with delivery"
          data-testid="bold-proposal-send-deferred"
          style={{
            fontSize: 12,
            fontWeight: 800,
            color: "var(--cvb-ghost)",
      ...
      <div style={{ padding: "22px 40px 40px" }}>
        {tab === "Document" ? (
          <div
            style={{
              display: "grid",
              gridTemplateColumns: "minmax(320px,660px) 210px",
              gap: 26,
              alignItems: "start",
            }}
            data-testid="bold-proposal-doc"
          >
            <div>
            ...
            <div data-testid="bold-proposal-palette" style={{ opacity: 0.6 }}>
              ...
              {(
                ...
                >
                  ...
                  <span style={{ fontSize: 12, fontWeight: 700, color: "var(--cvb-faint)" }}>
                    {name}
                  </span>
                </div>
              ))}
        ...
        {tab === "Activity" ? (
          <div
            ...
            data-testid="bold-proposal-activity"
          >
            Opens, reading time and forwards land here once sending exists — a draft has no activity
            to report, and nothing here will ever be invented.
          </div>
        ) : null}
          ...
          >
            <div style={{ padding: "13px 15px", borderBottom: "1px solid var(--cvb-line-inner)" }}>
              ...
              <div style={{ fontSize: 11.5, color: "var(--cvb-ghost)", marginTop: 2 }}>
                {contactName ?? "No contact attached yet"}
              </div>
            </div>
            <div
              style={{
...
function Block({
  ...
  if (block.kind === "cover") {
    return (
      <div
        style={{
          background: "linear-gradient(150deg,#0C2A1B,#0A1524)",
          borderRadius: 18,
      ...
      >
        ...
        {block.eyebrow ? (
          <div
            style={{ ...mono, fontSize: 9, letterSpacing: ".18em", color: "rgba(255,255,255,.55)" }}
          >
            {block.eyebrow}
          </div>
        ) : null}
  ...
  if (block.kind === "text") {
    return (
      ...
      >
        {block.label ? (
          <div style={{ ...mono, fontSize: 9, letterSpacing: ".16em", color: "var(--cvb-faint)" }}>
            {block.label}
          </div>
        ) : null}
        {editing ? (
          <div style={{ marginTop: 8 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 8, justifyContent: "flex-end" }}>
              ...
              <span
                onClick={onSave}
                data-testid="bold-proposal-block-save"
                style={{
                  fontSize: 11.5,
                  fontWeight: 800,
                  color: "#fff",
          ...
          <>
            ...
            <div
              style={{ fontSize: 12.5, color: "var(--cvb-faint)", lineHeight: 1.6, marginTop: 6 }}
            >
              {block.body}
            </div>
            {onEdit ? (
              <span
                onClick={onEdit}
                data-testid="bold-proposal-block-edit"
                style={{
                  position: "absolute",
                  top: 12,
                  right: 14,
  ...
  if (block.kind === "price") {
    return (
      ...
      >
        ...
        <div style={{ fontWeight: 800, fontSize: 15, marginTop: 6, marginBottom: 10 }}>
          {block.title}
        </div>
        {block.options.map((o) => (
          <div
            ...
            style={{
              ...
              borderRadius: 12,
              marginTop: 6,
              background: o.best ? "var(--cvb-mint)" : "var(--cvb-panel)",
              border: `1px solid ${o.best ? "var(--cvb-mint-line)" : "var(--cvb-line-inner)"}`,
            }}
          >
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 12.5, fontWeight: 700 }}>{o.name}</div>
              {o.sub ? (
                <div style={{ fontSize: 11, color: "var(--cvb-ghost)", marginTop: 1 }}>{o.sub}</div>
              ) : null}
            </div>
            <span
              style={{
                ...
                fontWeight: 900,
                fontSize: 15,
                color: o.best ? "var(--cvb-forest)" : "var(--cvb-ink,#101613)",
              }}
            >
              {o.amount}
            </span>
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 14, fontWeight: 800, color: "var(--cvb-faint)", marginTop: 8 }}>
        Sign here
      </div>
      {block.body ? (
        <div style={{ fontSize: 11.5, color: "var(--cvb-ghost)", marginTop: 6 }}>{block.body}</div>
      ) : null}
      <div style={{ ...mono, fontSize: 9, color: "var(--cvb-ghost)", marginTop: 8 }}>
        signing arrives with delivery
      </div>
    </div>
  );
}
