...
export function BoldFormsView({
  ...
  const load = useCallback(async () => {
    ...
    if (res) {
      ...
      onCounts(
        res.forms.length,
        res.forms.reduce((n, f) => n + (f.responses ?? 0), 0),
      );
    }
  }, [onCounts]);
  useEffect(() => {
          ...
          <BoldCardGrid>
            {rows.map((f, i) => (
              <BoldCoverCard
                ...
                kind={
                  typeof f.design.kind === "string"
                    ? f.design.kind.toUpperCase()
  ...
  return (
    <div data-testid="bold-form-detail">
      <BoldMetaStrip
        items={[
          [
            "RESPONSES",
            String(responses),
            form.status === "live" ? "and counting" : "while it was live",
          ],
      ...
      >
        <span
          onClick={back}
          data-testid="bold-form-back"
          style={{ cursor: "pointer", fontSize: 12.5, fontWeight: 700, color: "var(--cvb-faint)" }}
        >
          ← All forms
        </span>
        {TABS.map((t) => (
          <span
            ...
            style={{
              ...
              color: tab === t ? "var(--cvb-ink,#101613)" : "var(--cvb-faint)",
              borderBottom: tab === t ? "2px solid var(--cvb-forest)" : "2px solid transparent",
            }}
          >
            {t}
          </span>
        ...
        <span
          onClick={() =>
            void patch(
              ...
              form.status === "live"
                ? "Taken down — the page now says it is closed."
                : "Live — the hosted page is up.",
            )
          }
          ...
          style={{
            fontSize: 12,
            fontWeight: 800,
            color: form.status === "live" ? "var(--cvb-faint)" : "#fff",
            background: form.status === "live" ? "var(--cvb-panel)" : "var(--cvb-forest)",
            border:
              form.status === "live"
                ? "1px solid var(--cvb-line-ctl)"
                : "1px solid var(--cvb-forest)",
            borderRadius: 11,
            padding: "8px 14px",
            cursor: "pointer",
            marginBottom: 6,
      ...
      <div style={{ padding: "22px 40px 40px" }}>
            ...
            >
              <div
                style={{
                  height: 3,
                  background:
                    "var(--cvb-gradient-signature, linear-gradient(90deg,#36D7ED,#35E834 55%,#D0F56B))",
                }}
              />
        ...
        {tab === "Responses" ? (
          <div
            ...
            data-testid="bold-form-responses"
          >
            {submissions.length === 0 ? (
              <div style={{ padding: 22, fontSize: 12.5, color: "var(--cvb-faint)" }}>
                Nothing yet — responses land here the moment one arrives.
              </div>
            ) : (
              submissions.map((s, i) => (
                <div
                  ...
                  style={{
                    ...
                    padding: "13px 16px",
                    borderBottom:
                      i === submissions.length - 1 ? "none" : "1px solid var(--cvb-line-inner)",
        ...
        {tab === "Fields" ? (
          ...
          >
            {form.fields.map((f, i) => (
              <div
                ...
                style={{
                  ...
                  padding: "12px 15px",
                  borderBottom:
                    i === form.fields.length - 1 ? "none" : "1px solid var(--cvb-line-inner)",
                }}
              >
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: 11, color: "var(--cvb-ghost)", marginTop: 2 }}>
                    {f.type === "choice"
                      ? `Choice · ${(f.options ?? []).length} options`
                      : `${f.type === "longtext" ? "Long text" : f.type[0]!.toUpperCase() + f.type.slice(1)} · ${f.required ? "required" : "optional"}`}
                  </div>
                </div>
                <span
                  onClick={() => {
                    const fields: BoldFormField[] = form.fields.map((x) =>
                      x.key === f.key ? { ...x, required: !x.required } : x,
                    );
                    void patch(
                      form.id,
                      { fields },
                      f.required ? `${f.label} — optional` : `${f.label} — required`,
                    );
                  }}
                  ...
                  style={{
                    ...
                    borderRadius: 13,
                    flex: "none",
                    background: f.required ? "var(--cvb-forest)" : "var(--cvb-scrollbar)",
                    position: "relative",
                    cursor: "pointer",
                  }}
                >
                  <span
                    style={{
                      position: "absolute",
                      top: 3,
                      left: f.required ? 21 : 3,
                      width: 18,
                      height: 18,
                      borderRadius: "50%",
                      background: "#fff",
                      boxShadow: "0 1px 3px rgba(16,22,19,.2)",
            ...
            >
              The toggle is required-or-not. Adding and re-ordering fields arrives with the field
              editor.
            </div>
          </div>
        ) : null}
        ...
        {tab === "Where it goes" ? (
          <div
            ...
            data-testid="bold-form-routing"
          >
            <RouteRow
              label="New contacts land in"
              value={
                form.routing.campaignId
                  ? "The routed campaign — enrolled on submit"
                  : "No campaign — just a contact"
              }
              chip={form.routing.campaignId ? "Campaign" : "None"}
            />
            <RouteRow
              label="Tagged"
              value={
                form.routing.tag
                  ? `“${form.routing.tag}” on every contact this form writes`
                  : "No tag"
              }
              chip={form.routing.tag ? "Live" : "None"}
            />
            <RouteRow
              label="After submit, show"
              value={form.routing.redirectUrl ?? "The built-in thank-you note"}
              chip={form.routing.redirectUrl ? "Redirect" : "Default"}
            />
            <div
              style={{
                padding: "12px 15px",
            ...
            {form.status === "live" && form.publicId ? (
              <>
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "center", marginTop: 10 }}>
                  ...
                  >{`${typeof window !== "undefined" ? window.location.origin : ""}/f/${form.publicId}`}</code>
                  <span
                    onClick={() => {
              ...
              <div style={{ fontSize: 12.5, color: "var(--cvb-faint)" }}>
                Publish the form first — the hosted page and its link appear here.
              </div>
            )}
          </div>
        ) : null}
