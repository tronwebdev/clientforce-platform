...
export function BoldCampaignsView({
  ...
  return (
    <div style={{ padding: "26px 40px 40px" }} data-testid="bold-camps-page">
      ...
      {/* Suggested rows — BOTTOM of the list per the prototype (.concat after
          drafts), full row anatomy: amber spine · mint name pill · "—
          SUGGESTED" hero · 0% bar · amber "Ada's idea" status pill · filled
          forest "Start it". Dismissal lives on the rail block (proto rows
          carry no ✕). */}
      {suggestions.map((g) => (
        <div
          key={g.id}
        ...
        >
          ...
          <div style={{ width: 108, flex: "none" }}>
            <div className="cvb-display" style={{ fontWeight: 900, fontSize: 20, letterSpacing: "-.03em", color: "var(--cvb-faint)" }}>—</div>
            <div style={{ ...mono, fontSize: 9.5, letterSpacing: ".1em", color: "var(--cvb-faint)", marginTop: 3 }}>SUGGESTED</div>
          </div>
          <div style={{ width: 118, flex: "none" }}>
            <div style={{ height: 5, borderRadius: 3, background: "var(--cvb-line-inner)", overflow: "hidden" }}>
              <span style={{ display: "block", height: 5, width: "0%", background: "var(--cvb-line-ctl)", borderRadius: 3 }} />
            </div>
            <div style={{ fontSize: 11, color: "var(--cvb-faint)", marginTop: 6 }}>0% of goal</div>
          </div>
          <div style={{ display: "flex", gap: 5, flex: "none" }}>
            {g.channels.map((ch) => {
              const t = CH_TONES[ch] ?? CH_TONES.email!;
          ...
          <span style={{ fontSize: 10, fontWeight: 700, color: "var(--cvb-amber)", background: "var(--cvb-amber-bg)", border: "1px solid var(--cvb-amber-line)", borderRadius: 999, padding: "4px 10px", flex: "none" }}>
            Ada's idea
          </span>
          <span
            onClick={(e) => {
            ...
            style={{ fontSize: 11.5, fontWeight: 800, color: "var(--cvb-card)", background: "var(--cvb-forest)", borderRadius: 10, padding: "8px 12px", cursor: "pointer", flex: "none" }}
          >
            Start it
          </span>
          <span style={{ ...mono, fontSize: 10, color: "var(--cvb-faint)", width: 62, flex: "none", textAlign: "right" }}>
