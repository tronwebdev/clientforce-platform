...
const CH_META: Record<string, { n: string; ic: string; bg: string; bd: string; fg: string }> = {
  email: { n: "Email", ic: "✉", bg: "var(--cvb-mint)", bd: "var(--cvb-mint-line)", fg: "var(--cvb-forest)" },
  sms: { n: "SMS", ic: "✆", bg: "var(--cvb-cyan-tint,#E2F3F6)", bd: "var(--cvb-cyan-line,#BFE3EB)", fg: "var(--cvb-cyan,#0E7D93)" },
  voice: { n: "Calls", ic: "☎", bg: "var(--cvb-panel)", bd: "var(--cvb-line-ctl)", fg: "var(--cvb-muted)" },
};
...
export function BoldStatsView({ agentId }: { agentId?: string }) {
  ...
  const tiles: Array<{ label: string; v: string; sub: string; fg?: string }> = [
    { label: "REACHED", v: loaded ? String(t?.reached ?? 0) : "—", sub: "unique contacts" },
    {
      ...
      sub: t?.repliedPct != null ? `${t.repliedPct}% of reached` : belowFloor && (t?.replied ?? 0) > 0 ? "too few sends to read a rate" : "of the people reached",
    },
    {
      label: "BOOKED",
      v: loaded ? String(t?.booked ?? 0) : "—",
      sub: t?.bookedPctOfRepliers != null ? `${t.bookedPctOfRepliers}% of repliers` : "moved to booked",
      fg: "var(--cvb-forest)",
    },
    t?.collectedCents
      ? { label: "COLLECTED", v: money(t.collectedCents), sub: "payments received", fg: "var(--cvb-forest-ink,#0E5C2B)" }
      : {
          label: "EST. VALUE",
  ...
  return (
    <div data-testid="bold-stats" style={{ padding: "26px 40px 40px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        <div style={{ display: "flex", gap: 5, background: "var(--cvb-panel)", borderRadius: 13, padding: 4, width: "fit-content" }}>
          {RANGES.map(([key, label]) => (
            <span
              key={key}
              onClick={() => setRange(key)}
              data-testid={`bold-stats-range-${key}`}
              style={{ fontSize: 12.5, fontWeight: 700, padding: "9px 16px", borderRadius: 10, cursor: "pointer", background: range === key ? "var(--cvb-card)" : "transparent", color: range === key ? "var(--cvb-ink)" : "var(--cvb-faint)" }}
            >
              {label}
            </span>
          ))}
        ...
        {workspaceScope ? (
          <>
            <select
              ...
              style={{ ...mono, fontSize: 11.5, fontWeight: 600, padding: "9px 12px", borderRadius: 11, background: "var(--cvb-card)", color: "var(--cvb-muted)", border: "1px solid var(--cvb-line-ctl)" }}
            >
            ...
            <span
              ...
              title="Coming soon"
              style={{ fontSize: 11.5, fontWeight: 600, padding: "9px 12px", borderRadius: 11, border: "1px dashed var(--cvb-line-ctl)", color: "var(--cvb-ghost)" }}
            >
      ...
      <div style={{ display: "flex", background: "var(--cvb-card)", border: "1px solid var(--cvb-line-ctl)", borderRadius: 20, overflow: "hidden", marginTop: 22 }}>
        {tiles.map((tile, i) => (
          <div key={tile.label} data-testid={`bold-stats-tile-${tile.label.toLowerCase().replace(/[^a-z]+/g, "-")}`} style={{ flex: 1, minWidth: 0, padding: "18px 14px", borderLeft: i === 0 ? "none" : "1px solid var(--cvb-line-inner)" }}>
            <div style={{ ...mono, fontSize: 10, letterSpacing: ".13em", color: "var(--cvb-faint)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{tile.label}</div>
            <div data-testid={`bold-stats-tilev-${tile.label.toLowerCase().replace(/[^a-z]+/g, "-")}`} style={{ fontWeight: 900, fontSize: 23, letterSpacing: "-.03em", lineHeight: 1, marginTop: 10, color: tile.fg ?? "var(--cvb-ink)", whiteSpace: "nowrap" }}>{tile.v}</div>
            <div style={{ fontSize: 10.5, color: "var(--cvb-faint)", marginTop: 8, lineHeight: 1.35 }}>{tile.sub}</div>
          </div>
        ))}
      </div>

      <div style={{ display: "flex", alignItems: "center", gap: 12, margin: "38px 0 20px" }}>
        <span style={{ ...mono, fontSize: 9.5, letterSpacing: ".18em", color: "var(--cvb-faint)" }}>WHERE PEOPLE DROP</span>
        <span style={{ flex: 1, height: 1, background: "var(--cvb-line-inner)" }} />
      </div>
      {loaded && (data?.tiles.reached ?? 0) === 0 ? (
        <div data-testid="bold-stats-empty" style={{ fontSize: 13, color: "var(--cvb-faint)", lineHeight: 1.6 }}>
      ...
      <div style={{ display: "flex", gap: 14, flexWrap: "wrap", marginTop: 32 }}>
        <div style={{ flex: 1, minWidth: 280, background: "var(--cvb-card)", border: "1px solid var(--cvb-line-ctl)", borderRadius: 20, padding: 22 }}>
          <div style={{ ...mono, fontSize: 9.5, letterSpacing: ".16em", color: "var(--cvb-faint)", marginBottom: 16 }}>BY CHANNEL</div>
          {(data?.channels ?? []).map((c, i, arr) => {
            const meta = CH_META[c.channel] ?? CH_META.email!;
            return (
              <div key={c.channel} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 0", borderBottom: i === arr.length - 1 ? "none" : "1px solid var(--cvb-line-inner)" }}>
                <span style={{ width: 30, height: 30, borderRadius: 11, flex: "none", background: meta.bg, border: `1px solid ${meta.bd}`, color: meta.fg, display: "grid", placeItems: "center", fontSize: 13 }}>{meta.ic}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 700, fontSize: 13.5, letterSpacing: "-.018em" }}>{meta.n}</div>
                  <div style={{ fontSize: 11, color: "var(--cvb-faint)", marginTop: 2 }}>
                    {c.sent} {c.channel === "voice" ? "placed" : "sent"} · {c.repliers} replied
                  </div>
                </div>
                <span style={{ fontWeight: 900, fontSize: 18, letterSpacing: "-.028em", color: c.booked > 0 ? "var(--cvb-forest)" : "var(--cvb-ghost)" }}>
                  {c.booked > 0 ? `${c.booked} booked` : "—"}
                </span>
              </div>
            );
          })}
          <div style={{ fontSize: 10.5, color: "var(--cvb-ghost)", marginTop: 12, lineHeight: 1.5 }}>
            Cost per booking arrives when sends and minutes start metering — no number is better than a made-up one.
          </div>
        </div>

        <div style={{ flex: 1, minWidth: 280, background: "var(--cvb-mint)", border: "1px solid var(--cvb-mint-line)", borderRadius: 20, padding: 22 }}>
          <div style={{ display: "flex", gap: 10 }}>
            <span style={{ color: "var(--cvb-forest)", fontSize: 14, flex: "none" }}>✦</span>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 900, fontSize: 17, letterSpacing: "-.028em", color: "var(--cvb-forest-ink,#0E3D22)" }}>What Ada sees</div>
              {loaded && (data?.reading.length ?? 0) === 0 ? (
                <div data-testid="bold-stats-reading-empty" style={{ fontSize: 13, color: "var(--cvb-forest)", lineHeight: 1.6, marginTop: 10 }}>
                  Nothing stands out in this window yet — readings appear when the numbers say something.
                </div>
              ) : (
                (data?.reading ?? []).map((line) => (
                  <div key={line} data-testid="bold-stats-reading" style={{ fontSize: 13.5, color: "var(--cvb-forest)", lineHeight: 1.6, marginTop: 10 }}>
                    {line}
                  </div>
                ))
              )}
              <div style={{ ...mono, fontSize: 9.5, color: "var(--cvb-forest)", opacity: 0.75, marginTop: 14, lineHeight: 1.5 }}>
