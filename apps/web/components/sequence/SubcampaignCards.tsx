...
export function stepPillText(n: StepNode): string {
  const raw =
    (n.mode === "guided" && n.brief
      ? n.brief.objective
      : n.content.subject?.trim() || n.content.body?.trim()) ?? "";
  ...
  const days = Math.round(
    chain.reduce(
      (acc, n) =>
        n.type === "delay"
          ? acc +
            (n.unit === "days" ? n.amount : n.unit === "hours" ? n.amount / 24 : n.amount / 1440)
          : acc,
      0,
    ),
  );
    ...
    <div data-testid="subcampaigns">
      {cards.length > 0 ? (
        <>
          ...
          <div
            style={{ display: "flex", alignItems: "center", gap: 12, margin: "18px 0 12px" }}
            data-testid="subcampaigns-divider"
          >
            <span style={{ fontSize: 12, fontWeight: 600, color: "#9AA59E", flex: "none" }}>
              ↳ {cards.length} sub-campaign{cards.length === 1 ? "" : "s"} based on contact
              behaviour
            </span>
            ...
            {cards.map((c) => (
              <div
                ...
                style={{
                  ...
                  minWidth: 0,
                  ...(expandedId === c.id ? { borderColor: "#9FD8AC" } : {}),
                }}
                data-testid="subcampaign-card"
              >
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                  {c.chip !== null ? (
                    <span
                      ...
                      data-testid="subcampaign-trigger-chip"
                    >
                      {c.chip}
                    </span>
                  ) : (
                  ...
                  {c.ai ? (
                    <span
                      ...
                      data-testid="subcampaign-ai-chip"
                    >
                      ✦ AI
                    </span>
                  ) : null}
                  ...
                  >
                    {c.pills.map((p, i) => (
                      ...
                      >
                        ...
                        {i < c.pills.length - 1 ? (
                          <span style={{ color: "#C2B79F", fontSize: 10 }}>→</span>
                        ) : null}
                      </span>
                    ))}
                  </div>
                ) : (
                  <div style={{ fontSize: 12, color: "#9AA59E", marginBottom: 7 }}>
                    No steps yet
                  </div>
                )}
                <div style={{ fontSize: 12, color: "#9AA59E" }} data-testid="subcampaign-meta">
                  {c.stepCount} step{c.stepCount === 1 ? "" : "s"} · {c.days} day
                  {c.days === 1 ? "" : "s"}
                </div>
              </div>
            ))}
      ...
      <div
        ...
        style={{
          ...
          alignItems: "center",
          gap: 12,
          border: `1.5px dashed ${addHover ? "#9FD8AC" : "#D8CFBE"}`,
          borderRadius: 13,
          background: addHover ? "rgba(53,232,52,.04)" : "transparent",
          padding: "13px 16px",
          marginTop: 12,
          cursor: "pointer",
        }}
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: "#9AA59E", marginTop: 2 }}>
            Create a branch triggered by a specific contact behaviour or rule.
          </div>
        </div>
