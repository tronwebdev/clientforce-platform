...
export function emailChecks(subject: string, body: string) {
  ...
  const grade = Math.min(
    ...
    Math.max(
      1,
      Math.round(
        0.39 * (words.length / sentences) + 11.8 * (syllables / Math.max(1, words.length)) - 15.59,
      ),
    ),
  );
  ...
  return [
    freeCount === 0
      ...
      : {
          label: `"Free" appears ${freeCount === 1 ? "once" : `${freeCount} times`}`,
          value: "Minor — consider rewording",
          ...warn,
        },
...
export interface StepEditorDrawerProps {
  ...
  modeControl?: {
    ...
    busy?: boolean;
    onFlip: (mode: "scripted" | "guided") => void;
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span
            style={{
              ...
              flex: "none",
              background:
                editBrief?.channel === "sms" ? "rgba(54,215,237,.16)" : "rgba(53,232,52,.16)",
              color: editBrief?.channel === "sms" ? "#1192A6" : "#16A82A",
              display: "flex",
              alignItems: "center",
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
              ...
              >
                {editStrategyIntent
                  ? `${intentTint(editStrategyIntent).label} reply`
                  : `Step ${editStepIndex}`}
              </span>
              {editBrief ? (
            ...
            >
              {editBrief ? editBrief.objective || "Untitled brief" : editSubject || "Untitled step"}
            </div>
          </div>
          <span
            onClick={onClose}
          ...
          >
            ...
            <div
              style={{
                ...
                borderRadius: 11,
                padding: 3,
                opacity: modeControl.busy ? 0.6 : 1,
              }}
            >
              <span
                onClick={
                  modeControl.busy || modeControl.mode === "scripted"
                    ? undefined
                    : () => modeControl.onFlip("scripted")
                }
                style={{
                  fontSize: 12.5,
                  fontWeight: modeControl.mode === "guided" ? 600 : 700,
                  color: modeControl.mode === "guided" ? "#8A7F6B" : "#0E1512",
                  background: modeControl.mode === "guided" ? "transparent" : "#fff",
                  boxShadow: modeControl.mode === "guided" ? "none" : "0 1px 4px rgba(14,21,18,.1)",
                  borderRadius: 9,
                  padding: "6px 12px",
                  cursor:
                    modeControl.busy || modeControl.mode === "scripted" ? "default" : "pointer",
                  whiteSpace: "nowrap",
                }}
                data-testid="step-mode-scripted"
              >
                Scripted
              </span>
              <span
                onClick={
                  modeControl.busy || modeControl.mode === "guided"
                    ? undefined
                    : () => modeControl.onFlip("guided")
                }
                style={{
                  fontSize: 12.5,
                  fontWeight: modeControl.mode === "guided" ? 700 : 600,
                  color: modeControl.mode === "guided" ? "#0E1512" : "#8A7F6B",
                  background: modeControl.mode === "guided" ? "#fff" : "transparent",
                  boxShadow: modeControl.mode === "guided" ? "0 1px 4px rgba(14,21,18,.1)" : "none",
                  borderRadius: 9,
                  padding: "6px 12px",
                  cursor: modeControl.busy || modeControl.mode === "guided" ? "default" : "pointer",
                  whiteSpace: "nowrap",
                }}
                data-testid="step-mode-guided"
              >
          ...
          >
            <div
              style={{
                ...
                border: "1px solid rgba(54,215,237,.28)",
                borderRadius: 11,
                padding: "11px 14px",
              }}
            ...
            >
              ...
              <span>
                This step has no fixed text. At send time the AI composes a fresh{" "}
                {editBrief.channel === "sms" ? "SMS" : "email"} for each lead from these talking
                points — checked against your never-say list,{" "}
                {editBrief.channel === "sms" ? "length" : "subject rules, length"} and grounding
                rules before anything sends.
                {editBrief.channel === "email"
                  ? " The unsubscribe footer is always added by the platform, never written by the AI."
                  : ""}{" "}
                {editBrief.channel === "sms" ? GUIDED_SMS_CREDITS : GUIDED_EMAIL_CREDITS} credits
                per send.
              </span>
            </div>
            ...
            <div>
              ...
              >
                Objective{seedMarks?.objective ? <SeedChip /> : null}
              </label>
              <input
                value={editBrief.objective}
                maxLength={200}
                onChange={(e) => setEditBrief((b) => (b ? { ...b, objective: e.target.value } : b))}
                placeholder="What must this message achieve?"
                style={{
                  ...
                  ...(seedMarks?.objective ? SEED_STYLE : {}),
                }}
            ...
            {editBrief.channel === "email" ? (
              <div>
                ...
                >
                  Subject hint <span style={{ fontWeight: 600, color: "#9AA59E" }}>· optional</span>
                  {seedMarks?.subjectHint ? <SeedChip /> : null}
                </label>
                <div style={{ fontSize: 12, color: "#9AA59E", marginBottom: 8 }}>
                  A direction for the subject line — the AI adapts it per lead. Subject rules (≤60
                  chars, no clickbait, no ALL CAPS) are checked on every composed email.
                </div>
                <input
                  value={editBrief.subjectHint}
                  maxLength={BRIEF_SUBJECT_HINT_MAX}
                  onChange={(e) =>
                    setEditBrief((b) => (b ? { ...b, subjectHint: e.target.value } : b))
                  }
                  placeholder="e.g. where phone-only booking leaks patients"
                  style={{
                    boxSizing: "border-box",
                    width: "100%",
                    borderRadius: 11,
                    ...
                    color: "#0E1512",
                    fontFamily: "'Hanken Grotesk',sans-serif",
                    ...(seedMarks?.subjectHint ? SEED_STYLE : {}),
                  }}
                  data-testid="brief-subject-hint"
                />
              </div>
            ) : null}
            ...
            <div>
              ...
              >
                Talking points{" "}
                <span style={{ fontWeight: 600, color: "#9AA59E" }}>
                  · {editBrief.talkingPoints.length} of {BRIEF_TALKING_POINTS_MAX} (min{" "}
                  {BRIEF_TALKING_POINTS_MIN})
                </span>
              </label>
              <div style={{ fontSize: 12, color: "#9AA59E", marginBottom: 8 }}>
                Facts the message may draw from — the AI picks what fits each lead, it never pastes
                them as-is.
              </div>
                ...
                {editBrief.talkingPoints.map((p, i) => (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: 11,
                      padding: "9px 12px",
                      ...(seedMarks?.points?.[i] ? SEED_STYLE : {}),
                    }}
                    data-testid="brief-point-row"
                  >
                    <span style={{ color: "#1192A6", flex: "none" }}>
                      {seedMarks?.points?.[i] ? "✦" : "•"}
                    </span>
                    <span style={{ fontSize: 13.5, color: "#3B463F", flex: 1, lineHeight: 1.45 }}>
                      {p}
                    </span>
                    <span
                      onClick={() =>
                        setEditBrief((b) =>
                          b
                            ? { ...b, talkingPoints: b.talkingPoints.filter((_, j) => j !== i) }
                            : b,
                        )
                      }
                      style={{
                      ...
                      data-testid="brief-point-remove"
                    >
                      ✕
                    </span>
                  </div>
              ...
              {editBrief.talkingPoints.length < BRIEF_TALKING_POINTS_MAX ? (
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
                      e.preventDefault();
                      const v = briefPointInput.trim();
                      if (v) {
                        setEditBrief((b) =>
                          b ? { ...b, talkingPoints: [...b.talkingPoints, v] } : b,
                        );
                        setBriefPointInput("");
                      }
                    }
                ...
                >
                  {BRIEF_TALKING_POINTS_MAX} of {BRIEF_TALKING_POINTS_MAX} — remove one to add
                  another.
                </div>
              )}
            </div>
            ...
            {[
              {
                key: "mustSay" as const,
                label: "Must say",
                desc: "Strings every composed message includes verbatim — keep for compliance-critical facts only.",
                max: BRIEF_MUST_SAY_MAX,
                input: briefMustInput,
                setInput: setBriefMustInput,
                tint: "#0F7A28",
                bg: "rgba(53,232,52,.09)",
                tid: "brief-must",
              },
              {
                key: "neverSay" as const,
                label: "Never say",
                desc: "Hard bans for this step — checked on every composed message before it can send.",
                max: BRIEF_NEVER_SAY_MAX,
                input: briefNeverInput,
                setInput: setBriefNeverInput,
                tint: "#C9543F",
                bg: "rgba(224,121,107,.08)",
                tid: "brief-never",
              },
            ].map((s) => (
              <div key={s.key}>
                ...
                >
                  {s.label} <span style={{ fontWeight: 600, color: "#9AA59E" }}>· optional</span>
                </label>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 7, marginBottom: 8 }}>
                    {editBrief[s.key].map((term) => (
                      <span
                        ...
                        data-testid={`${s.tid}-chip`}
                      >
                        {term}
                        <span
                          onClick={() =>
                            setEditBrief((b) =>
                              b ? { ...b, [s.key]: b[s.key].filter((x) => x !== term) } : b,
                            )
                          }
                          style={{
                            width: 17,
                ...
                {editBrief[s.key].length < s.max ? (
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        ...
                        if (
                          v &&
                          !editBrief[s.key].some((x) => x.toLowerCase() === v.toLowerCase())
                        ) {
                          setEditBrief((b) => (b ? { ...b, [s.key]: [...b[s.key], v] } : b));
                        }
                        s.setInput("");
                      }
                    }}
                  ...
                  >
                    {s.max} of {s.max} — remove one to add another.
                  </div>
                )}
              </div>
            ))}
            ...
            >
              <div
                style={{
                  ...
                  gap: 12,
                  padding: "13px 15px",
                  background: "linear-gradient(90deg,rgba(54,215,237,.1),rgba(53,232,52,.07))",
                  borderBottom: "1px solid #EBE3D6",
                }}
              >
                ...
                <span
                  onClick={() => void sampleCompose()}
                  style={{
                    fontSize: 12.5,
                    fontWeight: 700,
                    color: previewBusy ? "#9AA59E" : "#0A0F0C",
                    background: previewBusy ? "#ECE7DC" : GRAD,
                    borderRadius: 9,
                    padding: "7px 14px",
                    cursor: previewBusy ? "default" : "pointer",
                  }}
                  data-testid="sample-preview-run"
                >
                  {previewBusy ? "Composing…" : "Compose sample"}
                </span>
              </div>
              <div style={{ padding: "12px 15px" }}>
                {preview === null && !previewBusy ? (
                  <div style={{ fontSize: 12.5, color: "#9AA59E" }}>
                    See what the composer writes for a sample lead (Jane Doe · Acme Dental) using
                    the last saved brief. Free while guided mode is new.
                  </div>
                ) : previewBusy ? (
                  <div style={{ fontSize: 12.5, color: "#8A7F6B" }}>
                    Composing against the sample lead…
                  </div>
                  ...
                  <div data-testid="sample-preview-result">
                    ...
                    {preview.subject ? (
                      <div
                        ...
                        data-testid="sample-preview-subject"
                      >
                        {preview.subject}
                      </div>
                    ) : null}
                    ...
                    <div style={{ fontSize: 11.5, color: "#9AA59E", marginTop: 7 }}>
                      Sample lead: Jane Doe · Acme Dental — every real lead gets its own text.
                      {preview.subject
                        ? " The unsubscribe footer is appended at send time."
                        : ""}{" "}
                      {preview.credits} credits per real send (display only for now).
                    </div>
                  </div>
                  ...
                  <div
                    style={{
                      ...
                      borderRadius: 11,
                      background: "rgba(232,196,91,.08)",
                      padding: "11px 14px",
                    }}
                    data-testid="sample-preview-refused"
                  >
                    <div
                      style={{ fontSize: 12.5, fontWeight: 700, color: "#9A6B12", marginBottom: 3 }}
                    >
                      ⚠ Composer refused — nothing would send
                    </div>
                    <div style={{ fontSize: 12.5, color: "#8A7F6B", lineHeight: 1.5 }}>
                      {preview.reason}
                      {preview.detail ? ` — ${preview.detail}` : ""}. The same check pauses a real
                      lead instead of sending unchecked copy.
                    </div>
                  </div>
                ) : preview?.kind === "error" ? (
                  <div
                    style={{ fontSize: 12.5, color: "#C9543F" }}
                    data-testid="sample-preview-error"
                  >
                    {preview.message}
                  </div>
                ) : null}
          ...
          >
            ...
            {scriptedEmptyNote ? (
              <div
                ...
                data-testid="scripted-empty-note"
              >
                <span style={{ fontSize: 13 }}>✎</span>
                <span style={{ flex: 1 }}>{scriptedEmptyNote}</span>
                {composeDraft ? (
                  <span
                    onClick={composeDraft.busy ? undefined : composeDraft.run}
                    style={{
                      fontSize: 12.5,
                      fontWeight: 700,
                      color: composeDraft.busy ? "#9AA59E" : "#0A0F0C",
                      background: composeDraft.busy ? "#ECE7DC" : GRAD,
                      borderRadius: 9,
                      padding: "6px 12px",
                      cursor: composeDraft.busy ? "default" : "pointer",
                      flex: "none",
                      whiteSpace: "nowrap",
                    }}
                    data-testid="compose-draft"
                  >
                    {composeDraft.busy ? "Composing…" : "✦ Compose a draft"}
                  </span>
                ) : null}
              </div>
            ) : null}
            <div>
              ...
              >
                Subject line{seedMarks?.subject ? <SeedChip /> : null}
              </label>
              <input
                value={editSubject}
            ...
            <div>
              ...
              >
                Body{seedMarks?.body ? <SeedChip /> : null}
              </label>
              <textarea
                value={editBody}
                ...
                style={{
                  ...
                  resize: "vertical",
                  fontFamily: "'Hanken Grotesk',sans-serif",
                  ...(seedMarks?.body ? SEED_STYLE : {}),
                }}
                data-testid="edit-body"
              />
              <div style={{ fontSize: 11.5, color: "#9AA59E", marginTop: 6 }}>
                The signature and compliance footer are added at send time.
              </div>
            </div>

            {/* deliverability — deterministic rows only (AI-only score/verdict omitted) */}
            <div
              style={{ border: "1px solid #EBE3D6", borderRadius: 13, overflow: "hidden" }}
              data-testid="deliverability-card"
            >
              <div
                style={{
                  display: "flex",
                  ...
                  gap: 12,
                  padding: "13px 15px",
                  background: "linear-gradient(90deg,rgba(53,232,52,.1),rgba(54,215,237,.07))",
                  borderBottom: "1px solid #EBE3D6",
                }}
              >
                <span style={{ fontSize: 13, fontWeight: 700, color: "#0E1512", flex: 1 }}>
                  ✦ AI deliverability check
                </span>
              </div>
                ...
                {TOKENS.map((t) => (
                  <span
                    ...
                    style={{
                      ...
                      border: "1px solid rgba(54,215,237,.3)",
                      borderRadius: 8,
                      padding: "5px 10px",
                      cursor: "pointer",
                    }}
                    data-testid={`token-${t.replace(/[^a-zA-Z]/g, "")}`}
                  >
                    {t}
                  </span>
                ))}
                  ...
                  .map((d) => {
                    ...
                    return (
                      <span
                        key={d.id}
                        onClick={() => {
                          setCustomTokenKey(on ? null : d.key);
                          setCustomFallback("");
                        }}
                        style={{
                          fontSize: 12.5,
                          fontWeight: 600,
                          color: "#1192A6",
                          background: on ? "rgba(54,215,237,.24)" : "rgba(54,215,237,.12)",
                          border: `1px solid ${on ? "#36D7ED" : "rgba(54,215,237,.3)"}`,
                          borderRadius: 8,
                          padding: "5px 10px",
                          cursor: "pointer",
                        }}
                        data-testid={`token-custom-${d.key}`}
                      >{`{{custom.${d.key}}}`}</span>
                    );
                  })}
              </div>
              {customTokenKey ? (
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 7 }}>
                    ...
                    >{`Fallback for {{custom.${customTokenKey}}}`}</span>
                    <span
                      style={{
                        fontSize: 10,
                        fontWeight: 800,
                  ...
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") insertCustomToken();
                    }}
                  ...
                  <div style={{ fontSize: 11.5, color: "#5C6B62", lineHeight: 1.5, marginTop: 7 }}>
                    Used when a contact has no{" "}
                    <strong>
                      {fieldDefs.find((d) => d.key === customTokenKey)?.label ?? customTokenKey}
                    </strong>{" "}
                    value — custom tokens never render blank.
                  </div>
                  <span
                    onClick={insertCustomToken}
                    style={{
                      display: "inline-block",
                      marginTop: 9,
                      fontSize: 12.5,
                      fontWeight: 700,
                      color: customFallback.trim() ? "#0A0F0C" : "#9AA59E",
                      background: customFallback.trim() ? GRAD : "#ECE7DC",
                      borderRadius: 9,
                      padding: "7px 14px",
                      cursor: customFallback.trim() ? "pointer" : "not-allowed",
                    }}
                    data-testid="fallback-insert"
                  >
                    Insert token
                  </span>
                </div>
        ...
        >
          {editBrief ? (
            <span style={{ fontSize: 12.5, color: "#9AA59E" }}>
              Bullets steer the AI — the copy itself is written per lead.
            </span>
          ) : (
            <span
              title="AI rewrite arrives with the sequence tools — use ✦ Regenerate for a full re-plan"
              style={{
                fontSize: 13,
                fontWeight: 700,
                color: "#16A82A",
          ...
          >
            {editBrief ? "Save brief" : "Save step"}
          </span>
        </div>
      </div>
    </div>
