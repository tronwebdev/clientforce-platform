...
export function PricingView({ agencies }: { agencies: BackofficeAgencyRow[] }) {
  ...
  const refresh = useCallback(async (agencyId: string) => {
    const res = (await bo(`credit-prices${agencyId ? `?agencyId=${agencyId}` : ""}`).catch(
      () => null,
    )) as PricesResponse | null;
    if (res) setData(res);
  }, []);
  ...
  const submit = async (e: FormEvent) => {
    ...
    if (!Number.isInteger(n) || n < 0)
      return setError("Credits must be a non-negative whole number.");
    setBusy(true);
    try {
  ...
  const scopeLabel = scope
    ? (agencies.find((a) => a.id === scope)?.name ?? "agency")
    : "Platform defaults";
  ...
  return (
    <div>
      ...
      <p style={{ color: "#5b6560", fontSize: 14, margin: "0 0 18px", maxWidth: 720 }}>
        Effective-dated prices. Saving appends a new row (never edits in place), so the full change
        history is preserved and a per-agency override beats the platform default. Every change is
        audited.
      </p>
        ...
        <section>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              ...
              <tbody>
                {(data?.effective ?? []).length === 0 ? (
                  <tr>
                    <td colSpan={2} style={{ padding: 18, color: "#8a938d" }}>
                      No prices set.
                    </td>
                  </tr>
                ) : (
          ...
          >
            ...
            <input
              ...
              placeholder="action (e.g. email_send)"
              style={inp}
            />
            <input
              value={credits}
            ...
            />
            {error ? (
              <div style={{ color: "var(--cf-color-danger, #c9543f)", fontSize: 13 }}>{error}</div>
            ) : null}
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              ...
              <tbody>
                {(data?.history ?? []).length === 0 ? (
                  <tr>
                    <td colSpan={4} style={{ padding: 18, color: "#8a938d" }}>
                      No history.
                    </td>
                  </tr>
                ) : (
                  data!.history.map((r) => (
                    ...
                    >
                      ...
                      <td style={{ padding: "9px 14px", color: "#5b6560" }}>
                        {r.agencyId ? "override" : "default"}
                      </td>
