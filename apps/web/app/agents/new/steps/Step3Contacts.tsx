...
interface Step3Props {
  ...
  setCsvImport: React.Dispatch<
    React.SetStateAction<{ listId: string; name: string; count: number } | null>
  >;
  ...
  setPickedList: React.Dispatch<
    React.SetStateAction<{ id: string; name: string; memberCount: number } | null>
  >;
  listSearch: string;
...
export function Step3Contacts(props: Step3Props) {
  ...
  const cards = [
    csvImport
      ? {
          icon: "⬆",
          title: csvImport.name,
          desc: `${csvImport.count} contact${csvImport.count === 1 ? "" : "s"} enroll at launch · as of launch day`,
          iconbg: "rgba(53,232,52,.16)",
          act: () => setImportOpen(true),
          tid: "contacts-csv",
          picked: true,
          clear: () => setCsvImport(null),
          clearTid: "csv-import-clear",
          tag: false,
        }
      : {
          icon: "⬆",
    ...
    pickedList
      ? {
          icon: "❒",
          title: pickedList.name,
          desc: `${pickedList.memberCount} contact${pickedList.memberCount === 1 ? "" : "s"} enroll at launch · as of launch day`,
          iconbg: "rgba(54,215,237,.16)",
          act: () => {
            setListSearch("");
            setListOpen(true);
          },
          tid: "contacts-list",
          picked: true,
          clear: () => setPickedList(null),
          clearTid: "picked-list-clear",
          tag: false,
        }
      : {
          icon: "❒",
          title: "Choose a list",
          desc: "Pick an existing saved list.",
          iconbg: "rgba(54,215,237,.16)",
          act: () => {
            setListSearch("");
            setListOpen(true);
        ...
        },
    {
      icon: "✎",
      title: "Add manually",
      desc: "Enter contacts one by one.",
      iconbg: "#F2EEE4",
      act: () => setManualOpen(true),
      tid: "contacts-manual",
      picked: false,
      clear: null,
      clearTid: "",
              ...
              style={{
                ...
                border: c.picked
                  ...
                  : c.tag
                    ...
                    : "1px solid #EBE3D6",
                borderRadius: 14,
                background: c.picked ? "rgba(53,232,52,.07)" : "#fff",
                padding: 18,
                cursor: "pointer",
        ...
        >
          ...
          >
            ...
            {audienceTotal > 0 ? (
              ...
              >
                {audienceTotal} contact{audienceTotal === 1 ? "" : "s"} ready
              </span>
            ) : null}
          </div>
          {audienceTotal === 0 ? (
            <div
              style={{ padding: "26px 18px", textAlign: "center", fontSize: 13, color: "#9AA59E" }}
              data-testid="audience-empty"
            >
              No contacts yet — upload a CSV, choose a list, or add them manually.
            </div>
          ) : (
            <>
              ...
              {audienceTotal > shown ? (
                <div
                  ...
                  data-testid="audience-more"
                >
                  + {audienceTotal - shown} more
                </div>
              ) : null}
            </>
          )}
            ...
            >
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: 12.5, color: "#9AA59E" }}>
                  Pick a saved contact list to enroll.
                </div>
              </div>
              <span
                onClick={() => setListOpen(false)}
            ...
            <div style={{ padding: "14px 16px" }}>
              ...
              >
                ...
                <input
                  ...
                  data-testid="list-picker-search"
                />
              </div>
              {wizardLists.filter((l) => !l.archived).length === 0 ? (
                ...
                >
                  ...
                  <div
                    style={{ fontSize: 13.5, fontWeight: 700, color: "#0E1512", marginBottom: 3 }}
                  >
                    No saved lists yet
                  </div>
                  <div style={{ fontSize: 12.5, color: "#8A7F6B" }}>
                    Lists you save from Contacts appear here — upload a CSV or add contacts manually
                    for now.
                  </div>
                </div>
              ) : (
                  ...
                  {wizardLists
                    ...
                    .map((l) => (
                      <div
                        key={l.id}
                        onClick={() => {
                          setPickedList({ id: l.id, name: l.name, memberCount: l.memberCount });
                          setListOpen(false);
                        }}
                        onMouseEnter={(e) => {
                          e.currentTarget.style.borderColor = "#9FD8AC";
                          e.currentTarget.style.background = "#FBF7F0";
                        }}
                        onMouseLeave={(e) => {
                          e.currentTarget.style.borderColor = "#EBE3D6";
      ...
      {manualOpen ? (
        <div style={{ position: "fixed", inset: 0, zIndex: 60 }}>
          <div
            ...
            style={{ position: "absolute", inset: 0, background: "rgba(12,20,15,.4)" }}
          />
          <div
            style={{
              position: "absolute",
          ...
          >
            ...
            <div style={{ flex: 1, overflowY: "auto", minHeight: 0, padding: "20px 22px" }}>
              ...
              >
                ...
                <div
                  onClick={() => {
                    if (!manual.email.includes("@")) return;
                    setManualQueue((q) => [...q, manual]);
                  ...
                  style={{
                    ...
                    border: "1.5px solid rgba(53,232,52,.3)",
                    borderRadius: 11,
                    padding: 11,
                    cursor: manual.email.includes("@") ? "pointer" : "default",
                    opacity: manual.email.includes("@") ? 1 : 0.6,
                  }}
              ...
              >
                Added this session · {manualQueue.length}
              </div>
              {manualQueue.map((c, i) => (
                <div
                  key={`${c.email}-${i}`}
                ...
                >
                  <span
                    style={{
                      ...
                      borderRadius: "50%",
                      flex: "none",
                      background: i % 2 === 0 ? "rgba(53,232,52,.16)" : "rgba(54,215,237,.16)",
                      color: "#0A0F0C",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                  ...
                  >
                    {`${(c.firstName.replace(/^dr\.?\s+/i, "")[0] ?? "").toUpperCase()}${(c.lastName[0] ?? "").toUpperCase()}` ||
                  ...
                  <span
                    onClick={() => setManualQueue((q) => q.filter((_, j) => j !== i))}
                    style={{
                      color: "#C9543F",
                      fontSize: 12,
                      fontWeight: 600,
            ...
            >
              <span style={{ fontSize: 13, color: "#9AA59E", flex: 1 }}>
                {manualQueue.length} contact{manualQueue.length === 1 ? "" : "s"} ready to add
              </span>
              <span
                onClick={() => {
                  if (manualQueue.length === 0) return;
                  void addContacts(manualQueue, "manual").then(() => {
                    setManualQueue([]);
                    setManualOpen(false);
                  });
                }}
                style={{
                  ...
                  borderRadius: 11,
                  padding: "10px 22px",
                  cursor: manualQueue.length ? "pointer" : "default",
                  boxShadow: "0 6px 16px rgba(53,232,52,.26)",
                  opacity: manualQueue.length ? 1 : 0.55,
                }}
