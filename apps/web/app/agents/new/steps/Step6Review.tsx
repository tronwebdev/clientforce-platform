...
export function Step6Review(props: Step6Props) {
  ...
  const costRows: { label: string; value: string }[] = [
    {
      label: `Outbound sends (${channelNames.join(" · ") || "Email"})`,
      value: `${outboundTotal.toLocaleString()} credits`,
    },
    ...
    ...(capture.enabled && apOn
      ? [
          { label: "Auto-prospecting", value: `${CREDIT_PRICES.signal_lead} credits / found lead` },
          {
            ...
            value: `${CREDIT_PRICES.enrichment} credits / enriched lead`,
          },
        ]
      : []),
  ...
  const captureRows: { label: string; value: string; on: boolean }[] = [
    { label: "Auto-prospecting", value: apOn ? "On" : "Off", on: apOn },
    {
      ...
      value: capture.widget ? "On — pick an asset" : "Off",
      on: capture.widget,
    },
    {
      label: "Hosted form link",
      value: capture.form ? "On — pick an asset" : "Off",
      on: capture.form,
    },
    {
      label: "Embed on website",
      value: capture.embed ? "On — pick an asset" : "Off",
      on: capture.embed,
    },
  ];
  ...
  const guardRows: { label: string; value: string }[] = [
    ...
    { label: "Suppression & Do-Not-Contact", value: "Honored" },
    {
      ...
      value: `${daysLabel(sendDays)} · ${h12(windowStart)}–${h12(windowEnd)} ${tzShort(timezone)}`,
    },
    {
      label: "Daily send limits",
      value: `${(dailyCap + (hasSms ? smsDailyCap : 0)).toLocaleString()} / day`,
    },
  ];
    ...
    <div style={{ maxWidth: 820 }}>
      ...
      >
        {[
          ...
          {
            label: "Channels",
            value: `${channelNames.length || 1} channel${channelNames.length === 1 ? "" : "s"}`,
          },
          { label: "Sequence", value: `${steps.length} step${steps.length === 1 ? "" : "s"}` },
          {
            label: "Audience",
            value: `${audienceTotal.toLocaleString()} contact${audienceTotal === 1 ? "" : "s"}`,
          },
        ].map((c) => (
          <div
            key={c.label}
            ...
            data-testid={`review-tile-${c.label.toLowerCase()}`}
          >
            <div style={{ fontSize: 12.5, color: "#8A7F6B", fontWeight: 600, marginBottom: 8 }}>
              {c.label}
            </div>
            <div
              style={{
      ...
      <div
        ...
        data-testid="review-capture-card"
      >
        ...
        {captureRows.map((c) => (
          <div
            ...
            data-testid="review-capture-row"
          >
            ...
            <span
              style={{
                fontSize: 13.5,
                fontWeight: 600,
                color: capture.enabled && c.on ? "#16A82A" : "#9AA59E",
              }}
            >
              {capture.enabled ? c.value : "Off"}
            </span>
          </div>
        ))}
      </div>
      ...
      <div
        ...
        data-testid="review-guardrails-card"
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
          <span
            style={{
        ...
        {guardRows.map((g) => (
          <div
            ...
            data-testid="review-guardrail-row"
          >
      ...
      >
        ...
        >
          ...
          <span
            ...
            data-testid="review-cost-total"
          >
            {outboundTotal.toLocaleString()}{" "}
            <span style={{ fontSize: 14, color: "#8A7F6B", fontWeight: 600 }}>credits</span>
          </span>
        </div>
      </div>
      ...
      {allResolved ? (
        <div
          ...
          data-testid="launch-strip"
        >
          <span style={{ fontSize: 14, color: "#5C6B62", flex: 1 }}>
            ✓ Everything the agent needs is resolved —{" "}
            <strong style={{ color: "#0E1512" }}>ready to launch</strong>.
          </span>
          <span
            onClick={() => void launch()}
            style={{
              ...
              borderRadius: 12,
              padding: "13px 28px",
              boxShadow: "0 6px 18px rgba(53,232,52,.3)",
              cursor: "pointer",
              flex: "none",
            }}
            data-testid="launch-strip-deploy"
          >
            🚀 Deploy agent
          </span>
        </div>
        ...
        >
          <span style={{ fontSize: 14, color: "#5C6B62", flex: 1 }}>
            ✦ {gapTotal - gapResolved} unresolved gap{gapTotal - gapResolved === 1 ? "" : "s"} —
            resolve them in step 1 (type it or let AI decide) before launching.
          </span>
          <span
            title="Resolve every gap before launching"
            style={{
              fontFamily: "'Hanken Grotesk',sans-serif",
              fontWeight: 700,
              fontSize: 16,
            ...
            data-testid="launch-strip-deploy-disabled"
          >
            🚀 Deploy agent
          </span>
        </div>
