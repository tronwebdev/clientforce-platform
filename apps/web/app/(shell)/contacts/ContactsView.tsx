  ...
  return (
    ...
    <div style={{ display: "flex", minWidth: 0, flex: 1, lineHeight: "normal" }}>
      ...
      <div style={{ flex: 1, background: "#FBF7F0", minWidth: 0, padding: "24px 28px 30px", boxSizing: "border-box" }}>
        ...
        {/* C2.5 import — THE shared 3-step flow (W3-1: one implementation,
            mounted here and in the Create-Agent wizard; it stays mounted while
            closed so a DEC-058 background run survives the modal closing). */}
        <ContactImportFlow
          open={importOpen}
          onClose={() => setImportOpen(false)}
          lists={activeLists}
          fieldDefs={activeDefs}
          refreshDefs={() => void refreshDefs()}
          isAdmin={isAdmin}
          existingRows={rows}
          toast={setToastMsg}
          onImported={() => {
