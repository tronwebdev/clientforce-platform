  ...
  return (
    <div style={{ marginBottom: 13 }}>
      ...
      {options && !loading && !error && (
        ...
        >
          {options.length === 0 && (
            <div style={{ padding: "11px 14px", fontSize: 13, color: "#9AA59E" }}>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  cursor: disabled ? "not-allowed" : "pointer",
                  background: on ? "rgba(53,232,52,.08)" : "transparent",
                }}
              >
                <span
                  style={{ fontSize: 14, fontWeight: on ? 700 : 600, color: "#0E1512", flex: 1 }}
                >
                  #{c.name}
                </span>
            ...
            return (
              ...
              >
                <span style={{ minWidth: 0, flex: 1 }}>
                  ...
                  {c.timeZone && (
                    <span style={{ display: "block", fontSize: 11.5, color: "#9AA59E" }}>
                      {c.timeZone}
                    </span>
                  )}
                </span>
                {on && (
                  <span style={{ color: "#16A82A", fontSize: 13, fontWeight: 700, flex: "none" }}>
                    ✓
                  </span>
                )}
              </div>
...
export function IntegrationDrawer({
  ...
  useEffect(() => {
    ...
    setDraft({
      ...
      toggles: Object.fromEntries(
        slackContent.syncRows.map((r) => [r.kind, notificationOn(config, r.kind)]),
      ) as Record<SlackNotificationKind, boolean>,
    });
  ...
  const startOAuth = useCallback(async () => {
    ...
    try {
      const data = (await cf(`integrations/${entry.id}/connect`, { method: "POST" })) as {
        authorizeUrl: string;
      };
  ...
  const connectFields = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      if (isStripe) {
        body = {};
        if (stripeFields.paymentLinkUrl.trim())
          body.paymentLinkUrl = stripeFields.paymentLinkUrl.trim();
        ...
        if (hubspotFields.defaultPipeline.trim())
          body.defaultPipeline = hubspotFields.defaultPipeline.trim();
      } else {
        body = {};
      ...
      await cf(`integrations/${entry.id}/connect-fields`, {
        method: "POST",
      ...
      setFieldsError(
        err instanceof CfError && err.detail ? err.detail : "Couldn't connect — try again",
      );
      return false;
    } finally {
      setFieldsBusy(false);
  ...
  const loadOptions = useCallback(async () => {
    ...
    try {
      const data = (await cf(`integrations/${entry.id}/options?kind=${content.optionsKind}`)) as {
        options: OptionRow[];
      };
      ...
      setOptionsError(
        err instanceof CfError && err.detail
          ? err.detail
          : `Couldn't load ${content.optionsKind} — try again`,
      );
    } finally {
  ...
  const saveConfig = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      await cf(`integrations/${entry.id}`, {
        method: "PATCH",
  ...
  const syncNow = useCallback(async () => {
    ...
    setProbeErr(null);
    try {
      const res = (await cf(`integrations/${entry.id}/probe`, { method: "POST" })) as {
        status: string;
        detail: string;
      };
    ...
    async (kind: SlackNotificationKind) => {
      ...
      try {
        const payload = slackConfigPayload(config, {
          notifications: { [kind]: !notificationOn(config, kind) },
        });
        await cf(`integrations/${entry.id}`, {
          method: "PATCH",
          body: JSON.stringify({ config: payload }),
        ...
        setActionError(
          err instanceof CfError && err.detail ? err.detail : "Couldn't update — try again",
        );
      } finally {
        setToggleBusy(null);
      }
  ...
  const disconnect = useCallback(async () => {
    ...
    try {
      ...
      setActionError(
        err instanceof CfError && err.detail ? err.detail : "Couldn't disconnect — try again",
      );
      setDiscBusy(false);
      setDiscArmed(false);
    }
  ...
  useEffect(() => {
    ...
    const fetchActivity = async () => {
      try {
        const data = (await cf(`integrations/${entry.id}/activity`)) as {
          deliveries: ActivityDelivery[];
          events: ActivityEvent[];
        };
        if (live) {
  ...
  const draftTogglesUI = (
    <div>
      {slackContent.syncRows.map((r) => (
        ...
        >
          <span style={{ fontSize: 13.5, fontWeight: 600, color: "#0E1512", flex: 1 }}>
            {r.label}
          </span>
          <ToggleSwitch
            ...
            onToggle={() =>
              setDraft((d) => ({ ...d, toggles: { ...d.toggles, [r.kind]: !d.toggles[r.kind] } }))
            }
          />
        </div>
      ))}
    ...
    <>
      <CalendarPicker
        ...
        onPick={(c) =>
          ...
          setGcalDraft((d) => ({
            ...d,
            calendar: { id: c.id, name: c.name, timeZone: c.timeZone ?? "UTC" },
          }))
        }
        onRetry={() => {
          setOptions(null);
      ...
      >
        <span style={{ fontSize: 13.5, fontWeight: 600, color: "#0E1512", flex: 1 }}>
          Offer open slots in composed copy
        </span>
        <ToggleSwitch
  ...
  const calendlyFieldsUI = (
    <>
      ...
      <div style={{ fontSize: 12, color: "#9AA59E", lineHeight: 1.5 }}>
        Booking detection needs an API token from Calendly → Integrations → API &amp; Webhooks (paid
        Calendly plans). The scheduling link works without it.
      </div>
    </>
  );
  ...
  const stripeFieldsUI = (
    <>
      ...
      <div style={{ fontSize: 12, color: "#9AA59E", lineHeight: 1.5 }}>
        Payment detection needs a restricted key from Stripe → Developers → API keys with Webhook
        Endpoints write. The payment link works without it.
      </div>
    </>
  );
  ...
  const webhooksFieldsUI = (
    <>
      ...
      <div style={{ fontSize: 12, color: "#9AA59E", lineHeight: 1.5 }}>
        A public https endpoint you operate — Clientforce sends a SIGNED test delivery on connect; a
        2xx from your receiver confirms it. Private and internal addresses are refused by the
        delivery guard.
      </div>
    </>
  );
    ...
    <>
      <div style={{ marginBottom: 13 }}>
        ...
        <input
          ...
          placeholder="Private-app token or Service Key"
          style={FIELD_INPUT}
        />
      </div>
      <div style={{ marginBottom: 13 }}>
        ...
        >
          Default pipeline <span style={{ color: "#9AA59E", fontWeight: 500 }}>(optional)</span>
        </label>
        <input
      ...
      <div style={{ fontSize: 12, color: "#9AA59E", lineHeight: 1.5 }}>
        Create a HubSpot Private App or Service Key (Settings → Integrations) with the
        <b> crm.objects.deals.write</b> and <b> crm.objects.contacts.write</b> scopes, then paste
        the token. One-way push — Clientforce never reads changes back from HubSpot.
      </div>
    </>
  );
  ...
  if (wizStep !== null) {
        ...
        ? wizStep === 1
          ? isStripe
            ...
            : isWebhooks
              ...
              : isHubspot
                ? "Paste a HubSpot token — one-way push of deals & stages."
    ...
    body = (
      <>
        <div style={{ fontSize: 13.5, color: "#5C6B62", lineHeight: 1.5, marginBottom: 18 }}>
          {entry.desc}
        </div>
        <span style={SECTION}>
          Step {wizStep} of {segCount}
        </span>
          ...
          {segs.map((s) => (
            <span
              ...
              style={{
                ...
                height: 5,
                borderRadius: 100,
                background: s <= wizStep ? "#16A82A" : "#E4EAE6",
              }}
            />
          ))}
        </div>
        <div style={{ fontFamily: BRICO, fontWeight: 700, fontSize: 17, color: "#0E1512" }}>
          {stepTitle}
        </div>
          ...
          <>
            <div
              ...
              style={{
                ...
                opacity: canManage && !connectBusy ? 1 : 0.6,
                marginBottom: 16,
              }}
            >
              <span
          ...
          <>
            ...
            >
              {isStripe ? (
                <>
                  <div
                    style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 15px" }}
                  >
                    <span
                      style={{
                        color: stripeFields.paymentLinkUrl.trim() ? "#16A82A" : "#C9CFC9",
                        fontSize: 13,
                      }}
                    >
                      {stripeFields.paymentLinkUrl.trim() ? "✓" : "○"}
                    </span>
                    <span
                      data-testid="confirm-stripe-link"
                      style={{
                        fontSize: 13.5,
                        color: "#3B463F",
                        overflow: "hidden",
                    ...
                    >
                      {stripeFields.paymentLinkUrl.trim()
                        ? `Payment link — ${stripeFields.paymentLinkUrl.trim()}`
                        : "No payment link — detection only"}
                    </span>
                  </div>
                  <div
                    style={{
                  ...
                  >
                    <span
                      style={{
                        color: stripeFields.apiKey.trim() ? "#16A82A" : "#C9CFC9",
                        fontSize: 13,
                      }}
                    >
                    ...
                    <span
                      data-testid="confirm-stripe-key"
                      style={{ fontSize: 13.5, color: "#3B463F" }}
                    >
                      {stripeFields.apiKey.trim()
                ...
                <>
                  ...
                  >
                    <span style={{ color: "#16A82A", fontSize: 13 }}>✓</span>
                    <span
                      data-testid="confirm-webhooks-url"
                      style={{
                        fontSize: 13.5,
                        color: "#3B463F",
                        overflow: "hidden",
                    ...
                    >
                      Payload URL — {whFields.defaultUrl.trim()}
                    </span>
                  </div>
                  <div
                    style={{
                  ...
                  >
                    ...
                    <span style={{ fontSize: 13.5, color: "#3B463F" }}>
                      A signed test event is POSTed on connect — only a 2xx confirms the receiver;
                      the signing secret appears on the drawer.
                    </span>
                  </div>
                </>
              ) : isHubspot ? (
                <>
                  ...
                  >
                    <span
                      style={{
                        color: hubspotFields.apiToken.trim() ? "#16A82A" : "#C9CFC9",
                        fontSize: 13,
                      }}
                    >
                      {hubspotFields.apiToken.trim() ? "✓" : "○"}
                    </span>
                    <span
                      data-testid="confirm-hubspot-token"
                      style={{ fontSize: 13.5, color: "#3B463F" }}
                    >
                      {hubspotFields.apiToken.trim()
                        ? "HubSpot token supplied — verified on connect"
                        : "Paste a HubSpot token to connect"}
                    </span>
                  </div>
                  <div
                    style={{
                  ...
                  >
                    <span
                      style={{
                        color: hubspotFields.defaultPipeline.trim() ? "#16A82A" : "#C9CFC9",
                        fontSize: 13,
                      }}
                    >
                ...
                <>
                  ...
                  >
                    ...
                    >
                      Scheduling link — {fields.schedulingUrl.trim()}
                    </span>
                  </div>
                  <div
                    style={{
                  ...
                  >
                    <span
                      style={{
                        color: fields.apiToken.trim() ? "#16A82A" : "#C9CFC9",
                        fontSize: 13,
                      }}
                    >
                ...
                <>
                  {gcalDraft.calendar && (
                    ...
                    >
                      <span style={{ color: "#16A82A", fontSize: 13 }}>✓</span>
                      <span style={{ fontSize: 13.5, color: "#3B463F" }}>
                        Availability reads from “{gcalDraft.calendar.name}” (
                        {gcalDraft.calendar.timeZone})
                      </span>
                    </div>
                  )}
                  <div
                    style={{
                      ...
                      gap: 10,
                      padding: "10px 15px",
                      borderTop: gcalDraft.calendar ? "1px solid #F2EEE4" : "none",
                    }}
                  >
                    <span
                      style={{ color: gcalDraft.offerSlots ? "#16A82A" : "#C9CFC9", fontSize: 13 }}
                    >
                      {gcalDraft.offerSlots ? "✓" : "○"}
                    </span>
                    <span style={{ fontSize: 13.5, color: "#3B463F" }}>
                      {gcalDraft.offerSlots
                        ? "Open slots may appear in composed copy"
                        : "Slots in copy off — the booking link is the mechanism"}
                    </span>
                  </div>
                  <div
                    style={{
                  ...
                  >
                    <span style={{ color: "#16A82A", fontSize: 13 }}>✓</span>
                    <span style={{ fontSize: 13.5, color: "#3B463F" }}>
                      Calendly puts booked meetings on this calendar
                    </span>
                  </div>
                </>
              ) : (
                <>
                  {draft.channel && (
                    ...
                    >
                      <span style={{ color: "#16A82A", fontSize: 13 }}>✓</span>
                      <span style={{ fontSize: 13.5, color: "#3B463F" }}>
                        Alerts post to #{draft.channel.name}
                      </span>
                    </div>
                  )}
                  {onNotifs.map((r) => (
                  ...
                  {onNotifs.length === 0 && (
                    <div
                      style={{
                        ...
                        fontSize: 13,
                        color: "#9AA59E",
                        borderTop: draft.channel ? "1px solid #F2EEE4" : "none",
                      }}
                    >
      ...
      content.mode === "fields"
        ? isStripe
          ...
          : isWebhooks
            ...
            : isHubspot
              ? "Paste your HubSpot token first"
        ...
        {content.mode === "oauth" && wizStep === 1 ? (
          ...
          <span
            ...
            style={{
              ...
              borderRadius: 11,
              padding: "10px 20px",
              cursor: canContinue && !wizBusy ? "pointer" : "not-allowed",
              opacity: canContinue && !wizBusy ? 1 : 0.6,
              boxShadow: "0 6px 16px rgba(53,232,52,.26)",
            }}
          >
      ...
      >
        ...
        <span
          ...
          onClick={() => {
            ...
            const run =
              isCalendly || isStripe || isWebhooks || isHubspot ? connectFields : saveConfig;
            void run().then((ok) => {
          ...
          style={{
            ...
            borderRadius: 11,
            padding: "10px 20px",
            cursor: canManage && !settingsBusy ? "pointer" : "not-allowed",
            opacity: canManage && !settingsBusy ? 1 : 0.6,
            boxShadow: "0 6px 16px rgba(53,232,52,.26)",
          }}
        >
          {isCalendly
    ...
    body = (
      <>
        ...
        {row.status === "revoked" && (
          <div
            style={{
              background: "rgba(224,121,107,.08)",
              border: "1px solid #F0CFC8",
              borderRadius: 14,
              padding: 14,
              marginBottom: 18,
        ...
        >
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
            <span style={{ fontSize: 13, fontWeight: 700, color: "#0E1512", flex: 1 }}>
              Connection
            </span>
            <span style={{ fontSize: 12, fontWeight: 700, color: health.color }}>
              {health.text}
            </span>
          </div>
          <div
          ...
          >
            ...
            <span style={{ fontSize: 13, fontWeight: 600, color: "#0E1512" }}>
              {row.accountLabel ?? "—"}
            </span>
          </div>
          <div
          ...
          <div style={{ marginTop: 12, display: "flex", alignItems: "center", gap: 10 }}>
            <span
              ...
              style={{
                ...
                borderRadius: 10,
                padding: "8px 14px",
                cursor: canManage && !probeBusy ? "pointer" : "not-allowed",
            ...
            {probeMsg && (
              <span
                data-testid="probe-detail"
                style={{ fontSize: 12, color: "#5C6B62", flex: 1, minWidth: 0 }}
              >
                {probeMsg}
              </span>
            )}
            {probeErr && (
              <span
                ...
                style={{ fontSize: 12, color: "#C9543F", flex: 1, minWidth: 0 }}
              >
                {probeErr}
              </span>
            )}
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 15px" }}>
              ...
              <span
                ...
                style={{
                  fontSize: 13.5,
                  fontWeight: 600,
                  color: config.channel ? "#0E1512" : "#9AA59E",
                  flex: 1,
                  textAlign: "right",
                }}
              >
            ...
            {slackContent.syncRows.map((r) => {
              ...
              return (
                ...
                >
                  <span style={{ color: on ? "#16A82A" : "#C9CFC9", fontSize: 13, flex: "none" }}>
                    {on ? "✓" : "○"}
                  </span>
        ...
        {isGcal && (
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 15px" }}>
              ...
              <span
                data-testid="calendar-value"
                style={{
                  fontSize: 13.5,
                  fontWeight: 600,
                  color: gcalConfig.calendar ? "#0E1512" : "#9AA59E",
                  flex: 1,
                  textAlign: "right",
                  overflow: "hidden",
                  textOverflow: "ellipsis",
              ...
              >
                {gcalConfig.calendar
                  ? `${gcalConfig.calendar.name} · ${gcalConfig.calendar.timeZone}`
                  : "Not picked yet"}
              </span>
              <span
            ...
            <div
              data-testid="sync-row-availability"
              style={{
                display: "flex",
                alignItems: "center",
                gap: 10,
            ...
            >
              <span
                style={{
                  color: gcalConfig.calendar ? "#16A82A" : "#C9CFC9",
                  fontSize: 13,
                  flex: "none",
                }}
              >
              ...
              <span style={{ fontSize: 13.5, color: "#3B463F", flex: 1 }}>
                Availability — open slots can appear in composed copy
              </span>
            </div>
            <div
              data-testid="sync-row-offer-slots"
              style={{
                display: "flex",
                alignItems: "center",
                gap: 10,
            ...
            >
              <span
                style={{
                  color: offerSlotsOn(gcalConfig) ? "#16A82A" : "#C9CFC9",
                  fontSize: 13,
                  flex: "none",
                }}
              >
                {offerSlotsOn(gcalConfig) ? "✓" : "○"}
              </span>
              <span style={{ fontSize: 13.5, color: "#3B463F", flex: 1 }}>
                Offer open slots in composed copy
              </span>
            ...
            >
              ...
              <span style={{ fontSize: 13.5, color: "#3B463F", flex: 1 }}>
                Bookings — Calendly puts booked meetings on this calendar
              </span>
            </div>
          </div>
        )}
        {isCalendly && (
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 15px" }}>
              <span style={{ fontSize: 12.5, color: "#9AA59E", flex: "none" }}>
                Scheduling link
              </span>
              <span
                ...
                style={{
                  ...
                  fontWeight: 600,
                  color: calConfig.schedulingUrl ? "#0E1512" : "#9AA59E",
                  flex: 1,
                  textAlign: "right",
                  overflow: "hidden",
                  textOverflow: "ellipsis",
              ...
              {calConfig.schedulingUrl && (
                <span
                  ...
                  onClick={() => copyText("link", calConfig.schedulingUrl!)}
                  style={{
                    fontSize: 12.5,
                    fontWeight: 700,
                    color: "#16A82A",
            ...
            <div
              data-testid="calendly-detection"
              style={{
                display: "flex",
                alignItems: "flex-start",
                gap: 10,
            ...
            >
              <span
                style={{
                  color: detection.detection ? "#16A82A" : "#C9CFC9",
                  fontSize: 13,
                  flex: "none",
                }}
              >
              ...
              <span
                style={{
                  fontSize: 13,
                  color: detection.detection ? "#16A82A" : "#5C6B62",
                  flex: 1,
                  lineHeight: 1.45,
                  fontWeight: detection.detection ? 600 : 400,
                }}
              >
                {detection.line}
              </span>
            ...
            {calConfig.webhookToken && (
              <div
                data-testid="calendly-webhook"
                style={{ padding: "10px 15px", borderTop: "1px solid #F2EEE4" }}
              >
                <div style={{ fontSize: 12.5, color: "#9AA59E", marginBottom: 4 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  >
                    {calendlyWebhookPath(calConfig.webhookToken)}
                  </span>
                  <span
                    data-testid="copy-webhook"
                    onClick={() =>
                      copyText("webhook", calendlyWebhookPath(calConfig.webhookToken!))
                    }
                    style={{
                      fontSize: 12.5,
                      fontWeight: 700,
                ...
                <div style={{ fontSize: 11.5, color: "#9AA59E", marginTop: 4, lineHeight: 1.45 }}>
                  Informational — Calendly posts booking events here; the detection state above is
                  what matters.
                </div>
              </div>
            )}
        ...
        {isStripe && (
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 15px" }}>
              ...
              <span
                data-testid="stripe-payment-link"
                style={{
                  fontSize: 12.5,
                  fontFamily: "monospace",
                  fontWeight: 600,
                  color: stripeCfg.paymentLinkUrl ? "#0E1512" : "#9AA59E",
                  flex: 1,
                  textAlign: "right",
                  overflow: "hidden",
                  textOverflow: "ellipsis",
              ...
              {stripeCfg.paymentLinkUrl && (
                <span
                  ...
                  onClick={() => copyText("paylink", stripeCfg.paymentLinkUrl!)}
                  style={{
                    fontSize: 12.5,
                    fontWeight: 700,
                    color: "#16A82A",
            ...
            >
              <span
                style={{
                  color: stripeDetection.detection ? "#16A82A" : "#C9CFC9",
                  fontSize: 13,
                  flex: "none",
                }}
              >
                {stripeDetection.detection ? "✓" : "○"}
              </span>
              <span
                style={{
                  fontSize: 13,
                  color: stripeDetection.detection ? "#16A82A" : "#5C6B62",
                  flex: 1,
                  lineHeight: 1.45,
                  fontWeight: stripeDetection.detection ? 600 : 400,
                }}
              >
                {stripeDetection.line}
              </span>
            ...
            {stripeCfg.webhookToken && (
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  >
                    {stripeWebhookPath(stripeCfg.webhookToken)}
                  </span>
                  <span
                    data-testid="copy-stripe-webhook"
                    onClick={() =>
                      copyText("stripe-webhook", stripeWebhookPath(stripeCfg.webhookToken!))
                    }
                    style={{
                      fontSize: 12.5,
                      fontWeight: 700,
                ...
                <div style={{ fontSize: 11.5, color: "#9AA59E", marginTop: 4, lineHeight: 1.45 }}>
                  Informational — Stripe posts checkout events here; the detection state above is
                  what matters.
                </div>
              </div>
            )}
        ...
        {isWebhooks && (
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 15px" }}>
              ...
              <span
                data-testid="webhooks-default-url"
                style={{
                  fontSize: 12.5,
                  fontFamily: "monospace",
                  fontWeight: 600,
                  color: whCfg.defaultUrl ? "#0E1512" : "#9AA59E",
                  flex: 1,
                  textAlign: "right",
                  overflow: "hidden",
                  textOverflow: "ellipsis",
              ...
              <span
                data-testid="webhooks-url-change"
                onClick={openSettings}
                style={{
                  fontSize: 12.5,
                  fontWeight: 700,
            ...
            {whCfg.signingSecret && (
              <div
                data-testid="webhooks-secret"
                style={{ padding: "10px 15px", borderTop: "1px solid #F2EEE4" }}
              >
                <div style={{ fontSize: 12.5, color: "#9AA59E", marginBottom: 4 }}>
                  Signing secret — verify every delivery with it
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <span
                    style={{
                  ...
                  <span
                    ...
                    onClick={() => copyText("secret", whCfg.signingSecret!)}
                    style={{
                      fontSize: 12.5,
                      fontWeight: 700,
                      color: "#16A82A",
        ...
        {isHubspot && (
          ...
          >
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 15px" }}>
              ...
              <span
                data-testid="hubspot-portal"
                style={{
                  fontSize: 12.5,
                  fontFamily: "monospace",
                  fontWeight: 600,
                  color: hubspotCfg.portalId ? "#0E1512" : "#9AA59E",
                  flex: 1,
                  textAlign: "right",
                }}
              >
            ...
            >
              ...
              <span
                data-testid="hubspot-pipeline-value"
                style={{
                  fontSize: 12.5,
                  fontWeight: 600,
                  color: hubspotCfg.defaultPipeline ? "#0E1512" : "#9AA59E",
                  flex: 1,
                  textAlign: "right",
                }}
              >
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 18 }}>
          {row.scopes.length === 0 && (
            <span style={{ fontSize: 12.5, color: "#9AA59E" }}>No scopes recorded.</span>
          )}
          {row.scopes.map((s) => (
            <span
              key={s}
        ...
        {content.setupSteps.map((st, i) => (
          <div key={st.title} style={{ display: "flex", gap: 13, paddingBottom: 14 }}>
            ...
            >
              ...
              {i < content.setupSteps.length - 1 && (
                <span style={{ flex: 1, width: 2, background: "#EBE3D6", marginTop: 4 }} />
              )}
            </div>
            <div style={{ flex: 1, minWidth: 0, paddingTop: 3 }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: "#0E1512" }}>{st.title}</div>
              <div style={{ fontSize: 12.5, color: "#8A7F6B", lineHeight: 1.45, marginTop: 2 }}>
                {st.desc}
              </div>
            </div>
          </div>
        ...
        {activityError && (
          <div style={{ fontSize: 13, color: "#C9543F", padding: "9px 0" }}>
            Couldn't load activity — retrying.
          </div>
        )}
        ...
        >
          <span
            ...
            style={{
              ...
              fontWeight: 600,
              color: "#C9543F",
              background: discArmed ? "rgba(224,121,107,.1)" : "#fff",
              border: "1px solid #F0CFC8",
              borderRadius: 11,
              padding: "10px 18px",
              cursor: canManage && !discBusy ? "pointer" : "not-allowed",
    ...
    body = (
      <div style={{ fontSize: 13.5, color: "#5C6B62" }}>
        ...
        <span
          ...
          style={{ color: "#16A82A", fontWeight: 700, cursor: "pointer" }}
        >
          Connect it
        </span>
      </div>
                  ...
                  {pill.pulse && (
                    <span
                      style={{
                        ...
                        borderRadius: "50%",
                        background: "#35E834",
                        boxShadow: "0 0 0 3px rgba(53,232,52,.25)",
                        flex: "none",
                      }}
                    />
                  )}
