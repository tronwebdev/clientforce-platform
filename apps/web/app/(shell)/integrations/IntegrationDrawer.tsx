...
export function IntegrationDrawer({ entry, provider, row, bootMode, canManage, onClose, onChanged }: {
  ...
  // INT W5 (DEC-101): the minted token is held in component state ONLY, for the
  ...
  const hubspotFieldsUI = (
    <>
      <div style={{ marginBottom: 13 }}>
        <label style={{ display: "block", fontSize: 12, fontWeight: 700, color: "#5C6B62", marginBottom: 6 }}>HubSpot token</label>
        <input
              ...
              {isStripe ? (
                ...
                <>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 15px" }}>
                    ...
                    <span data-testid="confirm-hubspot-token" style={{ fontSize: 13.5, color: "#3B463F" }}>{hubspotFields.apiToken.trim() ? "HubSpot token supplied — verified on connect" : "Paste a HubSpot token to connect"}</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 15px", borderTop: "1px solid #F2EEE4" }}>
