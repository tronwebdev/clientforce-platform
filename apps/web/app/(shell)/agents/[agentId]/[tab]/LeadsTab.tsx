...
> = {
  "lead.enrolled.v1": {
    ...
    bg: "#F2EEE4",
    fg: "#8A7F6B",
    label: () => "Enrolled in the sequence",
  },
  "email.sent.v1": {
    icon: "✉",
    bg: "#F2EEE4",
  ...
  "email.opened.v1": {
    ...
    bg: "#F2EEE4",
    fg: "#8A7F6B",
    label: (p) => `Opened${p.subject ? ` “${String(p.subject)}”` : ""}`,
  },
  "email.clicked.v1": {
    icon: "🔗",
  ...
  "email.compose_refused.v1": {
    ...
    fg: "#9A6B12",
    label: (p) =>
      `Email not sent — composer refused (${String(p.reason ?? "checks failed")}); lead paused`,
  },
  ...
  "lead.stage_changed.v1": {
    ...
    label: (p) =>
      p.label
        ? `Stage changed — ${String(p.label)}`
        : `Stage changed${p.fromStage ? ` — ${String(p.fromStage)} → ${String(p.toStage)}` : ""}`,
  },
  ...
  "sms.compose_refused.v1": {
    ...
    fg: "#9A6B12",
    label: (p) =>
      `SMS not sent — composer refused (${String(p.reason ?? "checks failed")}); lead paused`,
  },
  ...
  "calendar.booked.v1": {
    ...
    fg: "#16A82A",
    label: (p) => {
      const t = meetingTime(p.startAt);
      return `Meeting booked${t ? ` — ${t}` : ""}`;
    },
  },
  "calendar.rescheduled.v1": {
    ...
    fg: "#8A7F6B",
    label: (p) => {
      const t = meetingTime(p.toStartAt);
      return `Meeting rescheduled${t ? ` — now ${t}` : ""}`;
    },
  },
  "calendar.canceled.v1": {
    icon: "✕",
    bg: "rgba(224,121,107,.14)",
    fg: "#C9543F",
    label: (p) => {
      const t = meetingTime(p.startAt);
      return `${p.reason === "no_show" ? "Meeting no-show" : "Meeting canceled"}${t ? ` — was ${t}` : ""}`;
    },
  },
...
export function LeadsTab({
  ...
  const refreshLists = useCallback(async () => {
    ...
    if (view) {
      setContactLists(
        Object.fromEntries(
          (
            (view as { rows: { id: string; lists?: { id: string; name: string }[] }[] }).rows ?? []
          ).map((c) => [c.id, c.lists ?? []]),
        ),
      );
    }
  ...
  useEffect(() => {
    ...
    setTimeline(null);
    const load = async () => {
      const res = await cf(`agents/${agentId}/events?contactId=${open.contact.id}`).catch(
        () => null,
      );
  ...
  const counts = useMemo(() => {
    ...
    for (const f of FILTERS)
      if (f.id !== "all") c[f.id] = (leads ?? []).filter((l) => pillKey(l) === f.id).length;
    return c;
  }, [leads]);

  async function moveStage(enrollmentId: string, stage: string) {
    await cf(`enrollments/${enrollmentId}`, {
      method: "PATCH",
    ...
    <>
                ...
                {filters.map((f) => (
                  <div
                    ...
                    style={{
                      ...
                      background: filter === f.id ? "rgba(53,232,52,.06)" : "transparent",
                      cursor: "pointer",
                    }}
                  >
                    ...
                    <span
                      style={{
                        color: "#16A82A",
                        visibility: filter === f.id ? "visible" : "hidden",
                      }}
                    >
                      ✓
                    </span>
      ...
      {selected.length > 0 ? (
        ...
        >
          ...
          <span
            onClick={() => exportCsv(visible.filter((l) => checked[l.id]))}
            style={{
              fontSize: 13,
              fontWeight: 600,
              color: "#fff",
          ...
          <div style={{ position: "relative" }}>
            <span
              ...
              data-testid="leads-bulk-add-to-list"
            >
              ≣ Add to list
            </span>
            {bulkListDD ? (
              <AddToListMenu
                ...
                onPick={(listId) => {
                  void addLeadsToList(
                    listId,
                    visible.filter((l) => checked[l.id]).map((l) => l.contact.id),
                  );
                  setChecked({});
                }}
                onNewList={() =>
                  openNewList(visible.filter((l) => checked[l.id]).map((l) => l.contact.id))
                }
          ...
          <span
            onClick={() => {
              for (const id of selected) void moveStage(id, "interested");
              setChecked({});
            }}
            style={{
              fontSize: 13,
      ...
      >
        <div
          style={{
            display: "grid",
            gridTemplateColumns: "44px 1.9fr 1.3fr 1.1fr 1.05fr .7fr .9fr",
            alignItems: "center",
            padding: "0 8px",
            background: "#FBF7F0",
        ...
        >
          <div style={{ padding: "13px 12px" }}>
            <span
              onClick={() =>
                setChecked(allChecked ? {} : Object.fromEntries(visible.map((l) => [l.id, true])))
              }
              style={{
                display: "inline-flex",
                alignItems: "center",
                ...
                height: 18,
                borderRadius: 5,
                border: `2px solid ${allChecked ? "#35E834" : "#CDBFA8"}`,
                background: allChecked ? "linear-gradient(135deg,#36D7ED,#35E834)" : "transparent",
                color: "#fff",
                fontSize: 11,
                fontWeight: 800,
                cursor: "pointer",
        ...
        <div style={{ maxHeight: 512, overflowY: "auto" }}>
          {leads === null ? (
            ...
            >
              {(leads?.length ?? 0) === 0
                ? "No leads enrolled yet — launch the agent or add leads to begin."
                ...
                >
                  <div style={{ padding: "11px 12px" }} onClick={(e) => e.stopPropagation()}>
                    <span
                      ...
                      style={{
                        ...
                        height: 18,
                        borderRadius: 5,
                        border: `2px solid ${isChecked ? "#35E834" : "#CDBFA8"}`,
                        background: isChecked
                          ? "linear-gradient(135deg,#36D7ED,#35E834)"
                          : "transparent",
                        color: "#fff",
                        fontSize: 11,
                        fontWeight: 800,
                  ...
                  >
                    <span
                      style={{
                        ...
                        borderRadius: "50%",
                        flex: "none",
                        background: avTint(l.contact.id),
                        display: "flex",
                        alignItems: "center",
                        justifyContent: "center",
                        fontSize: 13,
                    ...
                    >
                      {initials(l.contact.firstName, l.contact.lastName, l.contact.email)}
                    </span>
                  ...
                  <div style={{ padding: "11px 12px" }}>
                    <span
                      ...
                      data-testid="lead-origin"
                    >
                      <span style={{ fontSize: 13, flex: "none" }}>{originCell(l).icon}</span>
                      {originCell(l).label}
                    </span>
                  </div>
                  <div style={{ padding: "11px 12px" }}>
                  ...
                  <div style={{ padding: "11px 12px", fontSize: 13, color: "#9AA59E" }}>
                    {timeAgo(l.updatedAt)}
                  </div>
                </div>
              );
      ...
      {open ? (
        <div
          ...
          style={{ position: "fixed", inset: 0, background: "rgba(12,20,15,.45)", zIndex: 60 }}
        >
          <div
            ...
            style={{
              ...
              width: 460,
              background: "#FBF7F0",
              boxShadow: "-24px 0 70px rgba(0,0,0,.32)",
              display: "flex",
          ...
          >
            ...
            >
              <span
                style={{
                  ...
                  borderRadius: "50%",
                  flex: "none",
                  background: avTint(open.contact.id),
                  display: "flex",
                  alignItems: "center",
                  justifyContent: "center",
                  fontSize: 16,
              ...
              >
                {initials(open.contact.firstName, open.contact.lastName, open.contact.email)}
              </span>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                >
                  {[open.contact.firstName, open.contact.lastName].filter(Boolean).join(" ") ||
                    open.contact.email}
                </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 7, marginTop: 7 }}>
                  <span
                    style={{
                      ...
                      fontSize: 12,
                      fontWeight: 600,
                      background: stagePill(pillKey(open)).bg,
                      color: stagePill(pillKey(open)).fg,
                    }}
                  >
                    {stagePill(pillKey(open)).label}
                  </span>
                  <span style={{ fontSize: 12, color: "#9AA59E" }}>
                    {stepIndex(open) ? `Step ${stepIndex(open)} of ${stepTotal}` : ""}
                  </span>
                </div>
              </div>
              <span
            ...
            <div style={{ flex: 1, overflow: "auto", padding: "18px 22px" }}>
                  ...
                  {moveDD ? (
                    ...
                    >
                      ...
                      {moveOptions(goalPill).map((m) => (
                        <div
                          key={m.stage}
                          onClick={() => void moveStage(open.id, m.stage)}
                          style={{
                            display: "flex",
                            alignItems: "center",
                            gap: 10,
                        ...
                        >
                          ...
                          <span
                            style={{
                              color: "#16A82A",
                              visibility: open.pipelineStage === m.stage ? "visible" : "hidden",
                            }}
                          >
                            ✓
                          </span>
                    ...
                    {drawerListDD ? (
                      <AddToListMenu
                        ...
                        options={lists.map((l) => ({
                          id: l.id,
                          name: l.name,
                          current: (contactLists[open.contact.id] ?? []).some((m) => m.id === l.id),
                        }))}
              ...
              >
                {[
                  ...
                  >
                    <span style={{ fontSize: 13, color: "#9AA59E", width: 96, flex: "none" }}>
                      {k}
                    </span>
                    <span
                      style={{
              ...
              {timeline === null ? (
                <div style={{ fontSize: 13, color: "#9AA59E" }} data-testid="timeline-skeleton">
                  Loading activity…
                </div>
              ) : timeline.length === 0 ? (
                <div style={{ fontSize: 13, color: "#9AA59E" }} data-testid="timeline-empty">
                  No activity yet.
                </div>
              ) : (
            ...
            >
              <span
                onClick={() => void moveStage(open.id, "unsubscribed")}
                style={{ fontSize: 13, fontWeight: 700, color: "#C9543F", cursor: "pointer" }}
              >
                Unsubscribe
              </span>
              <a
      ...
      {newListOpen ? (
        ...
        >
          <div
            ...
            data-testid="leads-new-list-modal"
          >
            <div
              style={{
                display: "flex",
            ...
            <div style={{ padding: "20px 22px" }}>
              ...
              <input
                ...
                data-testid="leads-new-list-name"
              />
              <div style={{ fontSize: 12.5, color: newListErr ? "#C9543F" : "#9AA59E" }}>
                {newListErr ??
                  (newListIds.length > 0
                    ? `${newListIds.length} lead${newListIds.length === 1 ? "" : "s"} will be added to it.`
            ...
            >
              ...
              <span
                ...
                data-testid="leads-new-list-create"
              >
                Create list
              </span>
            </div>
