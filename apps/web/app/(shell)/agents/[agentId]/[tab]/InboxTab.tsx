...
export function calendarSystemRow(
  ...
  switch (type) {
    ...
    case "calendar.rescheduled.v1": {
      ...
      return {
        text: `⟳ Meeting rescheduled${t ? ` — now ${t}` : ""}`,
        fg: "#8A7F6B",
        bg: "#F2EEE4",
        border: "#EBE3D6",
      };
    ...
    case "payment.received.v1": {
      ...
      if (amount !== null) {
        try {
          money = new Intl.NumberFormat("en-US", { style: "currency", currency: code }).format(
            amount / 100,
          );
        } catch {
      ...
      return {
        text: `💳 Payment received${money ? ` — ${money}` : ""}`,
        fg: "#16A82A",
        bg: "rgba(53,232,52,.1)",
        border: "rgba(53,232,52,.3)",
      };
...
export function InboxTab({ agentId, goalLabel }: { agentId: string; goalLabel?: string }) {
  const cats = INBOX_CATS.map((c) =>
    c.id === "booked" && goalLabel ? { ...c, label: goalLabel } : c,
  );
  ...
  const counts = useMemo(() => {
    ...
    for (const catDef of INBOX_CATS) {
      ...
      c[catDef.id] = (threads ?? []).filter(
        (t) => t.intent && catDef.intents.includes(t.intent),
      ).length;
    }
    return c;
  }, [threads]);
  ...
  const visible = useMemo(() => {
    let list = (threads ?? []).filter(
      (t) => cat === "all" || (t.intent ? catIntents.has(t.intent) : false),
    );
  ...
  const threadItems = useMemo(() => {
    ...
    const items: Array<{ key: string; at: string; msg?: ThreadMessage; ev?: ThreadEvent }> = [
      ...
      ...(sel.events ?? [])
        .filter(
          (e) => calendarSystemRow(e.type, (e.payload ?? {}) as Record<string, unknown>) !== null,
        )
  ...
  async function markDone() {
    ...
    await cf(`messages/${lastInbound.id}/done`, {
      method: "PATCH",
      body: JSON.stringify({ done: !sel.done }),
    }).catch(() => {});
    void refresh();
  }

  const heading =
    cat === "all" ? "All conversations" : (cats.find((c) => c.id === cat)?.label ?? "All");
  const sortLabel =
    sort === "newest" ? "Newest first" : sort === "oldest" ? "Oldest first" : "Name A–Z";
    ...
    <>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
              ...
              >
                ...
                <span
                  style={{
                    fontSize: 11,
                    fontWeight: 700,
                    color: on ? "#0A0F0C" : "#8A7F6B",
                    background: on ? "rgba(10,15,12,.14)" : "#F2EEE4",
                    borderRadius: 100,
                    padding: "1px 7px",
                  }}
                >
        ...
        <div style={{ position: "relative", flex: "none" }}>
          ...
          >
            <span style={{ fontSize: 13 }}>{chOpt.icon}</span>
            {chOpt.label}
            <span style={{ color: "#9AA59E", fontSize: 11, marginLeft: 2 }}>▾</span>
          </span>
          {channelDD ? (
            <div
              style={{
            ...
            >
              ...
              {CHANNEL_OPTIONS.map((o) => (
                ...
                >
                  ...
                  <span
                    style={{
                      color: "#16A82A",
                      visibility: channel === o.id ? "visible" : "hidden",
                    }}
                  >
                    ✓
                  </span>
              ...
              >
                ☎ Phone calls live in the <strong style={{ color: "#5C6B62" }}>Calls</strong> tab
              </div>
            </div>
          ) : null}
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "380px 1fr", gap: 16, height: 602 }}>
        ...
        >
          ...
          >
            ...
            >
              {heading}{" "}
              <span style={{ color: "#9AA59E", fontWeight: 600 }}>· {visible.length}</span>
            </span>
            <div style={{ position: "relative", flex: "none" }}>
              <span
                onClick={() => setSortDD((v) => !v)}
                style={{
                  fontSize: 13,
                  color: "#5C6B62",
                  fontWeight: 600,
                ...
                >
                  ...
                  {(["newest", "oldest", "name"] as const).map((s) => (
                    ...
                    >
                      ...
                      <span
                        style={{ color: "#16A82A", visibility: sort === s ? "visible" : "hidden" }}
                      >
                        ✓
                      </span>
                    </div>
                ...
                {[0, 1, 2, 3].map((i) => (
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      />
                      <div
                        style={{ height: 10, width: "80%", background: "#F7F2EA", borderRadius: 6 }}
                      />
                    </div>
                  </div>
                ))}
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      gap: 11,
                      padding: "12px 16px",
                      borderLeft: `3px solid ${on ? "#35E834" : "transparent"}`,
                      background: on ? "#FBF7F0" : "transparent",
                      cursor: "pointer",
                      borderBottom: "1px solid #F7F2EA",
                    ...
                    >
                      ...
                      <span
                        style={{
                          ...
                          height: 18,
                          borderRadius: "50%",
                          background: hasSms ? "rgba(54,215,237,.9)" : "rgba(53,232,52,.9)",
                      ...
                      >
                        <span
                          style={{
                            fontSize: 10.5,
                            fontWeight: 700,
                            color: tint?.fg ?? "#8A7F6B",
                            background: tint?.bg ?? "#F2EEE4",
                            borderRadius: 6,
                            padding: "2px 7px",
                          }}
                        >
                          {tint?.label ?? "Unclassified"}
                        </span>
                        {hasSms ? (
                          <span
                            style={{
            ...
            <>
              ...
              >
                <div style={{ minWidth: 0, flex: 1 }}>
                  ...
                  >
                    {sel.messages[0]?.subject ?? "(no subject)"}
                  </div>
                    ...
                    {(sel.channels?.length ? sel.channels : ["email"]).map((c) => (
                      <span
                        ...
                        style={{
                          fontSize: 11.5,
                          fontWeight: 700,
                          color: c === "sms" ? "#1192A6" : "#16A82A",
                          background: c === "sms" ? "rgba(54,215,237,.14)" : "rgba(53,232,52,.13)",
                          borderRadius: 6,
                          padding: "2px 9px",
                        }}
                      >
                    ...
                    {sel.intent && tintFor(sel.intent) ? (
                      <span
                        style={{
                          fontSize: 11.5,
                          fontWeight: 700,
                          color: tintFor(sel.intent)!.fg,
                          background: tintFor(sel.intent)!.bg,
                          borderRadius: 100,
                          padding: "3px 10px",
                        }}
                      >
                        {tintFor(sel.intent)!.label}
                      </span>
                    ) : null}
                    <span style={{ fontSize: 13, color: "#9AA59E" }}>
                      with{" "}
                      {[sel.contact?.firstName, sel.contact?.lastName].filter(Boolean).join(" ") ||
                        sel.contact?.email}
                    </span>
                  </div>
                </div>
                <span
                  ...
                  style={{
                    fontSize: 13,
                    color: sel.done ? "#8A7F6B" : "#16A82A",
                    fontWeight: 600,
                    border: `1px solid ${sel.done ? "#EBE3D6" : "#9FD8AC"}`,
                    background: sel.done ? "#fff" : "rgba(53,232,52,.06)",
                    borderRadius: 9,
                    padding: "7px 13px",
                    cursor: "pointer",
                    flex: "none",
                      ...
                      {[
                        {
                          ...
                          label: "Open lead",
                          act: () => {
                            window.location.href = `/agents/${agentId}/leads`;
                          },
                        },
                        {
                          icon: "✉",
                          ...
                          act: () => {
                            void navigator.clipboard?.writeText(sel.contact?.email ?? "");
                            setMenuOpen(false);
                          },
                        },
                        {
                          icon: "✓",
                          label: sel.done ? "Reopen conversation" : "Mark done",
                          act: () => {
                            void markDone();
                            setMenuOpen(false);
                          },
              ...
              <div
                style={{ overflowY: "auto", flex: 1, padding: 20, background: "#FCFAF6" }}
              ...
              >
                {threadItems.map((item) => {
                  ...
                  if (item.ev) {
                    const row = calendarSystemRow(
                      item.ev.type,
                      (item.ev.payload ?? {}) as Record<string, unknown>,
                    )!;
                    return (
                      <div
                        key={item.key}
                      ...
                      <div style={{ maxWidth: "80%", minWidth: 0 }}>
                        ...
                        >
                          <span
                            style={{
                              fontSize: 12.5,
                              fontWeight: 700,
                              color: inbound ? "#0E1512" : "#16A82A",
                            }}
                          >
                            {inbound
                              ? [sel.contact?.firstName, sel.contact?.lastName]
                                  .filter(Boolean)
                                  .join(" ") || sel.contact?.email
                              : "Agent"}
                          </span>
                          ...
                          {m.composed ? (
                            <span
                              ...
                              data-testid="msg-composed-tag"
                            >
                              ✦ Composed
                            </span>
                          ) : null}
                          <span style={{ fontSize: 11.5, color: "#9AA59E" }}>
                            {timeAgo(m.sentAt)}
                          </span>
                        </div>
                        <div
                          style={{
                            ...
                            lineHeight: 1.55,
                            color: "#243029",
                            background: inbound ? "#fff" : "rgba(53,232,52,.08)",
                            border: `1px solid ${inbound ? "#EBE3D6" : "rgba(53,232,52,.25)"}`,
                        ...
                        {m.composed ? (
                          <div
                            ...
                            data-testid="msg-composed-line"
                          >
                            composed from brief · checked against your rails
                          </div>
                        ) : null}
                      </div>
                    </div>
              ...
              >
                <div
                  style={{
                    ...
                    borderRadius: 14,
                    padding: "13px 15px",
                    boxShadow: "0 2px 8px rgba(14,21,18,.04)",
                  }}
                >
                  <div style={{ fontSize: 12.5, color: "#9AA59E", marginBottom: 8 }}>
                    Reply to {sel.contact?.email}
                  </div>
                  <div
                    style={{ fontSize: 14, color: "#B7BDB6", lineHeight: 1.5, marginBottom: 13 }}
                  >
                    Write a reply…
                    <span
                      style={{
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    <div
                      title="Reply sending arrives with a later unit"
                      style={{
                        marginLeft: "auto",
                        display: "flex",
                        alignItems: "stretch",
                    ...
                    >
                      ...
                      <span
                        style={{
                          ...
                          background: "#35E834",
                          padding: "11px 12px",
                          borderLeft: "1px solid rgba(10,15,12,.18)",
                        }}
                      >
                        ⌄
                      </span>
