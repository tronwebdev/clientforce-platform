 ...
 *   Interested/Voicemail need call tools + voicemail detection (Q-029).
...
export function receiptState(r: CallRetrievalDto): { label: string; fg: string; bg: string } {
  if (r.refusalReason)
    return { label: "Couldn't check", fg: "#C9543F", bg: "rgba(224,121,107,.16)" };
...
export function CallRetrievalsBlock({
  ...
  return (
    <div
      style={{ marginTop: 22, paddingTop: 18, borderTop: "1px solid #F2EEE4" }}
      data-testid="call-retrievals"
    >
      <div
        style={{
          fontSize: 11,
      ...
      {retrievals.map((r) => {
        ...
        return (
          <div
            ...
            data-testid="call-retrieval-row"
          >
            <span
              style={{
                fontSize: 11.5,
            ...
            <span
              ...
              data-testid="call-retrieval-state"
            >
              {state.label}
            </span>
          </div>
...
export default function CallsTab({ agentId }: { agentId: string }) {
  ...
  const connectRate =
    (calls ?? []).length > 0 ? `${Math.round((connected / (calls ?? []).length) * 100)}%` : "—";
  ...
  return (
    <div data-testid="calls-tab">
              ...
              >
                ...
                <span
                  style={{
                    fontSize: 11,
                    fontWeight: 700,
                    color: on ? "#0A3D16" : "#8A7F6B",
                    background: on ? "rgba(255,255,255,.45)" : "#F2EEE4",
                    borderRadius: 100,
                    padding: "1px 7px",
                  }}
                >
      ...
      {notice ? (
        <div
          data-testid="dial-notice"
          style={{
            marginBottom: 12,
            fontSize: 13,
            fontWeight: 600,
            color: "#8A6D1A",
            background: "rgba(232,196,91,.18)",
      ...
      {dialOpen ? (
        <div
          data-testid="dial-flyout"
          style={{
            marginBottom: 12,
            background: "#fff",
            border: "1px solid #EBE3D6",
            borderRadius: 14,
            boxShadow: "0 10px 28px rgba(14,21,18,.1)",
            padding: 14,
          }}
        >
          <div style={{ display: "flex", alignItems: "center", marginBottom: 10 }}>
            <span style={{ fontSize: 13.5, fontWeight: 700, color: "#0E1512", flex: 1 }}>
              Who should the agent call?
            </span>
            <button
              type="button"
          ...
          {contacts === null ? (
            ...
            <div style={{ fontSize: 13, color: "#8A7F6B" }}>
              No contacts with a phone number yet — add phones in Contacts first.
            </div>
          ) : (
            <div
              style={{
            ...
            >
              {contacts.map((c) => (
                <button
                  key={c.id}
                  type="button"
                  onClick={() => void dial(c.id)}
                  data-testid={`dial-contact-${c.id}`}
                  style={{
                    display: "flex",
                    alignItems: "center",
                    gap: 10,
      ...
      {calls === null ? (
        <div
          style={{ background: "#fff", border: "1px solid #EBE3D6", borderRadius: 16, padding: 20 }}
          data-testid="calls-skeleton"
        >
          {[0, 1, 2].map((i) => (
            <div
              key={i}
        ...
        >
          ...
          <div style={{ fontSize: 13.5, color: "#8A7F6B", maxWidth: 420, margin: "0 auto" }}>
            Start an AI call to a contact with a phone number — the transcript and outcome land
            here.
          </div>
        </div>
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "380px 1fr", gap: 16, height: 602 }}>
          ...
          >
            ...
            >
              ...
              <span style={{ fontSize: 12, fontWeight: 600, color: "#1192A6" }}>
                {connectRate} connect
              </span>
            </div>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      gap: 11,
                      padding: "13px 16px",
                      borderLeft: `3px solid ${isSel ? "#35E834" : o.accent}`,
                      background: isSel ? "rgba(53,232,52,.07)" : "#fff",
                      cursor: "pointer",
                  ...
                  >
                    ...
                    >
                      {initials(...(c.contactName.split(" ") as [string, string]))}
                    </span>
            ...
            {sel && detail ? (
              <>
                ...
                >
                  <span
                    style={{
                      ...
                      borderRadius: "50%",
                      flex: "none",
                      background: avTint(sel.contactId),
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "center",
                      fontSize: 15,
                  ...
                  >
                    {initials(detail.contact?.firstName, detail.contact?.lastName)}
                  </span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div
                      style={{
                    ...
                    <div style={{ fontSize: 12.5, color: "#9AA59E" }}>
                      {[sel.company, fmtDuration(sel.durationSec)].filter(Boolean).join(" · ")}
                    </div>
                  </div>
                  <span
                    style={{
                      fontSize: 11.5,
                      fontWeight: 700,
                      color: outcomeStyle(sel).fg,
                      background: outcomeStyle(sel).bg,
                      borderRadius: 7,
                      padding: "5px 11px",
                    }}
                  >
                    {outcomeStyle(sel).label}
                  </span>
                </div>
                ...
                <div style={{ overflowY: "auto", flex: 1, padding: "18px 20px" }}>
                  ...
                  {detail.transcript.length === 0 ? (
                    <div style={{ fontSize: 13, color: "#8A7F6B" }} data-testid="transcript-empty">
                      {sel.status === "QUEUED" || sel.status === "IN_PROGRESS"
                        ? "The transcript lands here when the call ends."
                    ...
                    detail.transcript.map((t) => (
                      <div
                        key={t.id}
                        style={{ display: "flex", gap: 11, marginBottom: 14 }}
                      ...
                      >
                        <span
                          style={{
                            fontSize: 11.5,
                            fontWeight: 700,
                            color: t.direction === "OUTBOUND" ? "#16A82A" : "#1192A6",
                            flex: "none",
                            width: 54,
                            paddingTop: 2,
                          }}
                        >
                          {t.direction === "OUTBOUND"
                            ? "Agent"
                            : detail.contact?.firstName || "Lead"}
                        </span>
                        <div
                          style={{ fontSize: 13.5, lineHeight: 1.55, color: "#3B463F", flex: 1 }}
                        >
                          {t.body}
                        </div>
                      </div>
                ...
                >
                  ...
                  <button
                    type="button"
                    onClick={() => void dial(sel.contactId)}
              ...
              >
                Select a call to see its transcript.
              </div>
            )}
          </div>
        </div>
