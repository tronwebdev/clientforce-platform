...
function describe(e: LogEvent): string {
  ...
  const p = e.payload ?? {};
  switch (e.type) {
    case "lead.enrolled.v1":
      return `${who} enrolled in the sequence.`;
    case "email.sent.v1":
      return `Step email sent to ${who}${p.subject ? ` — “${String(p.subject)}”` : ""}.`;
    case "email.delivered.v1":
      return `Email delivered to ${who}.`;
    case "email.opened.v1":
      return `${who} opened${p.subject ? ` “${String(p.subject)}”` : " an email"}.`;
    case "email.clicked.v1":
      return `${who} clicked a link.`;
    case "email.replied.v1":
      return `Reply received from ${who}${p.intent ? ` — classified “${intentTint(String(p.intent)).label}”` : ""}.`;
    case "email.bounced.v1":
      return `Email to ${who} hard-bounced.`;
    case "email.spam_reported.v1":
      return `${who} reported the email as spam.`;
    case "email.compose_refused.v1":
      return `Composer refused the email for ${who} — ${String(p.reason ?? "checks failed")}${p.detail ? ` (${String(p.detail)})` : ""}. The lead is paused; nothing was sent.`;
    case "lead.stage_changed.v1":
      return `${who} moved ${p.fromStage ? `from ${String(p.fromStage)} ` : ""}to ${String(p.toStage ?? "a new stage")}${p.manual ? " (manual move)" : ""}.`;
    case "lead.unsubscribed.v1":
      return `${who} unsubscribed — suppressed from all sequences.`;
    case "sms.sent.v1":
      return `Step SMS sent to ${who}.`;
    case "sms.delivered.v1":
      return `SMS delivered to ${who}.`;
    case "sms.failed.v1":
      return `SMS to ${who} failed${p.reason ? ` — ${String(p.reason)}` : ""}.`;
    case "sms.replied.v1":
      return `${who} replied by SMS${p.intent ? ` — classified “${String(p.intent)}”` : ""}.`;
    case "sms.opted_out.v1":
      return `${who} replied STOP — suppressed for SMS.`;
    case "sms.compose_refused.v1":
      return `Composer refused the SMS for ${who} — ${String(p.reason ?? "checks failed")}${p.detail ? ` (${String(p.detail)})` : ""}. The lead is paused; nothing was sent.`;
    case "call.started.v1":
      return `AI call to ${who} connected.`;
    case "call.completed.v1":
      return `AI call with ${who} completed${p.durationSec ? ` — ${Math.floor(Number(p.durationSec) / 60)}:${String(Number(p.durationSec) % 60).padStart(2, "0")}` : ""}. Transcript in the Calls tab.`;
    case "call.failed.v1":
      return `AI call to ${who} didn't complete${p.reason ? ` — ${String(p.reason).replace(/_/g, " ")}` : ""}.`;
    case "call.refused.v1":
      return `Dial to ${who} refused — ${String(p.reason ?? "rails blocked it")}. Nothing was dialed.`;
    case "voice.compose_refused.v1":
      return `A spoken turn for ${who} tripped its checks — ${String(p.reason ?? "check failed")}; the agent used the fallback line and the call continued.`;
    case "contact.enrollment_refused.v1":
      return `Enrollment refused for ${who} — ${p.reason === "CONTACT_INVALID" ? "invalid email address (list hygiene)" : String(p.reason ?? "refused")}${p.detail ? ` (${String(p.detail)})` : ""}. Nothing was enrolled or sent.`;
    // INT W2 (DEC-094): calendar copy — times render LOCAL from the payload.
    case "calendar.booked.v1": {
      const t = meetingTime(p.startAt);
      return `Meeting booked with ${who}${t ? ` — ${t}` : ""}.`;
    }
    case "calendar.rescheduled.v1": {
      const t = meetingTime(p.toStartAt);
      return `Meeting with ${who} rescheduled${t ? ` — now ${t}` : ""}.`;
    }
    case "calendar.canceled.v1": {
      const t = meetingTime(p.startAt);
      return p.reason === "no_show"
        ? `${who} didn't show for the meeting${t ? ` (${t})` : ""}.`
        : `Meeting with ${who} canceled${t ? ` (was ${t})` : ""}.`;
    }
    case "payment.received.v1": {
      const m = moneyLabel(p.amount, p.currency);
      return `Payment received from ${who}${m ? ` — ${m}` : ""}.`;
    }
    default:
      return `${e.type} — ${who}`;
  }
    ...
    return (
      ...
      >
        ...
        <div style={{ fontSize: 13.5, color: "#8A7F6B" }}>
          Every send, open, reply and stage move lands here the moment it happens.
        </div>
      </div>
    );
  }
        ...
        return (
          <div key={e.id} style={{ display: "flex", gap: 16 }} data-testid="log-row">
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
              ...
              {i < events.length - 1 ? (
                <span style={{ width: 2, flex: 1, background: "#E6E0D4" }} />
              ) : null}
            </div>
            <div style={{ paddingBottom: 20, flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#16A82A", marginBottom: 6 }}>
                {stamp(e.occurredAt)}
              </div>
              <div
                style={{
                  ...
                  borderRadius: 12,
                  padding: "13px 16px",
                  boxShadow: "0 2px 8px rgba(14,21,18,.03)",
                }}
              >
                {describe(e)}
              </div>
