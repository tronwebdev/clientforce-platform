      ...
      setAgents((list) =>
        list.map((a) =>
          ids.includes(a.id)
            ? {
                ...a,
                ...(body.status ? { status: body.status as AgentListItem["status"] } : {}),
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 15, color: "#5C6B62" }} data-testid="agents-subtitle">
            {rows.length} agents · {running} running · {totalContacts.toLocaleString()} contacts in
            sequences
          </div>
        </div>
        <a
      ...
      {selIds.length === 0 ? (
          ...
          >
            ...
            {search ? (
              <span
                ...
                style={{ color: "#9AA59E", cursor: "pointer", fontSize: 13 }}
              >
                ✕
              </span>
            ) : null}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 9, flex: "none" }}>
            <div style={{ position: "relative" }} data-dd>
              <div
                ...
                style={filterBtn(statusF !== "all")}
                data-testid="filter-status"
              >
                {statusF === "all" ? "Status" : statusDefs.find((d) => d.id === statusF)?.label}{" "}
                  ...
                  {statusDefs.map((o) => (
                    ...
                    >
                      ...
                      <span style={{ fontSize: 11, fontWeight: 700, color: "#8A7F6B" }}>
                        {o.id === "all"
                          ? rows.length
                          : rows.filter((a) => a.uiStatus === o.id).length}
                      </span>
                      <span
                        style={{
                          color: "#16A82A",
                          visibility: statusF === o.id ? "visible" : "hidden",
                        }}
                      >
                        ✓
                      </span>
            ...
            <div style={{ position: "relative" }} data-dd>
              <div onClick={() => toggleDD("channel")} style={filterBtn(channelF !== "all")}>
                {channelF === "all" ? "Channel" : channelDefs.find((d) => d.id === channelF)?.label}{" "}
                <span style={{ color: "#9AA59E", fontSize: 12 }}>⌄</span>
              </div>
              {open === "channel" ? (
                <div style={{ ...dd, width: 190 }}>
                  {channelDefs.map((o) => (
                    ...
                    >
                      ...
                      <span
                        style={{
                          color: "#16A82A",
                          visibility: channelF === o.id ? "visible" : "hidden",
                        }}
                      >
                        ✓
                      </span>
              ...
              <div onClick={() => toggleDD("more")} style={filterBtn(healthF !== "all")}>
                ...
                {healthF !== "all" ? (
                  <span style={{ fontSize: 11, color: "#9AA59E" }}>
                    · {healthDefs.find((d) => d.id === healthF)?.label}
                  </span>
                ) : null}{" "}
                <span style={{ color: "#9AA59E", fontSize: 12 }}>⌄</span>
              </div>
                  ...
                  {healthDefs.map((o) => (
                    ...
                    >
                      ...
                      <span
                        style={{
                          color: "#16A82A",
                          visibility: healthF === o.id ? "visible" : "hidden",
                        }}
                      >
                        ✓
                      </span>
            ...
            <div style={{ position: "relative" }} data-dd>
              <div
                onClick={() => toggleDD("cols")}
                style={filterBtn(colCount > 0)}
              ...
              {open === "cols" ? (
                <div style={{ ...dd, width: 236, padding: 6 }}>
                  ...
                  {[
                    ...
                    <div
                      key={c.key}
                      onClick={() => setColsOn((v) => ({ ...v, [c.key]: !v[c.key] }))}
                      style={{
                        display: "flex",
                        alignItems: "center",
                        gap: 10,
        ...
        >
          ...
          <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 9 }}>
            <span
              ...
              onClick={() =>
                patchAgents(
                  selIds,
                  { status: "ACTIVE" },
                  `${selIds.length} agent${selIds.length > 1 ? "s" : ""} resumed`,
                )
              }
            >
              ▶ Resume
            ...
            <span
              ...
              onClick={() =>
                patchAgents(
                  selIds,
                  { status: "PAUSED" },
                  `${selIds.length} agent${selIds.length > 1 ? "s" : ""} paused`,
                )
              }
            >
              ⏸ Pause
            ...
            <span
              style={bulkBtn()}
              onClick={() => setToast("Duplicate arrives with a later phase")}
            >
              ⧉ Duplicate
            </span>
            <span style={bulkBtn()} onClick={() => setToast(`Exporting ${selIds.length} agents…`)}>
              ↥ Export
            </span>
            <span style={bulkBtn(true)} onClick={() => deleteAgents(selIds)}>
              🗑 Delete
            </span>
          </div>
        </div>
      ...
      >
        ...
        >
          ...
          {visible.map((c) => (
            <div
              key={c.key}
              onClick={c.sortable ? () => doSort(c.sortKey!) : undefined}
              style={{
                padding: "14px 12px",
                fontSize: 12,
                fontWeight: 700,
            ...
            >
              {c.label}{" "}
              <span style={{ color: "#16A82A" }}>
                {c.sortable && sortKey === c.sortKey ? (sortDir === 1 ? "↑" : "↓") : ""}
              </span>
            </div>
          ))}
          <div />
        ...
        {pageRows.map((a, i) => {
          ...
          return (
            <div
              ...
              onClick={() =>
                router.push(
                  a.uiStatus === "Draft" ? `/agents/new?agent=${a.id}` : `/agents/${a.id}`,
                )
              }
              style={{
                display: "grid",
            ...
            >
              ...
              {visible.map((c) => (
                <div key={c.key} style={{ padding: 12, minWidth: 0, textAlign: c.align }}>
                  {c.kind === "agent" ? (
                    ...
                    >
                      <span
                        style={{ width: 6, height: 6, borderRadius: "50%", background: st.sdot }}
                      />
                      {a.uiStatus}
                    </span>
                  ) : c.kind === "channels" ? (
                    ...
                    >
                      <span
                        style={{ width: 8, height: 8, borderRadius: "50%", background: he.hdot }}
                      />
                      {a.health}
                    </span>
                  ) : (
                    (() => {
                      const val =
                        c.key === "payments"
                          ? 0
                          : ((a as unknown as Record<string, number>)[c.key] ?? 0);
                      return (
                        <span
                          style={{
                            fontSize: 14.5,
                            fontWeight: c.key === "contacts" ? 600 : 400,
                            color: val === 0 ? "#C2B79F" : "#0E1512",
                          }}
                        >
                          {val === 0 ? "—" : String(val)}
                        </span>
              ...
              >
                <span
                  onClick={() =>
                    patchAgents(
                      ...
                      `Agent ${isRunning ? "paused" : a.uiStatus === "Draft" ? "launched" : "resumed"}`,
                    )
                  }
                  ...
                  style={{
                    ...
                    alignItems: "center",
                    justifyContent: "center",
                    color: a.uiStatus === "Draft" ? "#16A82A" : isRunning ? "#C9543F" : "#16A82A",
                    fontSize: 13,
                    cursor: "pointer",
                  }}
                >
                ...
                <span
                  onClick={() => setRowMenu((v) => (v === a.id ? null : a.id))}
                  style={{
                    width: 30,
                    height: 30,
                    borderRadius: 9,
                  ...
                  >
                    {[
                        ...
                        ? [
                            {
                              ...
                              danger: false,
                              act: () => router.push(`/agents/new?agent=${a.id}`),
                            },
                          ]
                        : []),
                      {
                      ...
                      },
                      {
                        ...
                        act: () => {
                          setRowMenu(null);
                          setToast("Duplicate arrives with a later phase");
                        },
                      },
                      {
                        icon: "✏",
                        ...
                        act: () => {
                          ...
                          if (name?.trim())
                            void patchAgents([a.id], { name: name.trim() }, "Agent renamed");
                        },
                      },
                      {
                        icon: a.uiStatus === "Draft" ? "▶" : isRunning ? "⏸" : "▶",
                        label: a.uiStatus === "Draft" ? "Launch" : isRunning ? "Pause" : "Resume",
                        danger: false,
                        act: () =>
                          patchAgents(
                            [a.id],
                            { status: isRunning ? "PAUSED" : "ACTIVE" },
                            `Agent ${isRunning ? "paused" : "resumed"}`,
                          ),
                      },
                      {
                        icon: "↪",
                        ...
                        act: () => {
                          setRowMenu(null);
                          setToast("Folders arrive with a later phase");
                        },
                      },
        ...
        {total === 0 && rows.length > 0 ? (
          <div
            style={{ padding: "56px 20px", textAlign: "center", borderTop: "1px solid #F2EEE4" }}
            data-testid="filtered-empty"
          >
            <div style={{ fontSize: 30, marginBottom: 8 }}>🔍</div>
            <div style={{ fontSize: 15, fontWeight: 700, color: "#0E1512", marginBottom: 3 }}>
              No agents match
            </div>
          ...
          >
            <div
              style={{
                ...
                borderRadius: 24,
                background:
                  "linear-gradient(135deg,rgba(54,215,237,.18),rgba(53,232,52,.18) 55%,rgba(208,245,107,.3))",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                fontSize: 42,
            ...
            <div style={{ fontSize: 14.5, color: "#5C6B62", maxWidth: 420, marginBottom: 22 }}>
              Spin up your first AI agent to find leads, run outreach across every channel, and book
              calls on autopilot.
            </div>
            <a
              href="/agents/new"
                ...
                {open === "perPage" ? (
                  ...
                  >
                    {[8, 15, 25, 50].map((n) => (
                      <div
                        key={n}
                        style={{ ...ddRow, padding: "9px 14px", fontSize: 13 }}
                        onClick={() => {
                          setPerPage(n);
                          setOpen(null);
                          setPage(1);
              ...
              {Array.from({ length: pages }, (_, i) => i + 1).map((p) => (
                <span
                  ...
                  style={{
                    ...
                    padding: "0 6px",
                    borderRadius: 9,
                    border: `1px solid ${p === cur ? "#0C140F" : "#EBE3D6"}`,
                    background: p === cur ? "#0C140F" : "#fff",
                    color: p === cur ? "#fff" : "#0E1512",
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    fontSize: 14,
