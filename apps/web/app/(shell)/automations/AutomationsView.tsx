...
export function AutomationsView({ role }: { role: Role }) {
  ...
  const toggle = useCallback(
    async (row: AutomationListRow) => {
      ...
      try {
        ...
        setRows(
          (prev) =>
            prev?.map((r) => (r.id === row.id ? { ...r, enabled: row.enabled } : r)) ?? prev,
        );
        flash(
  ...
  const merged = rows ?? [];
  const countFor = (s: Seg) =>
    s === "All" ? merged.length : merged.filter((r) => statusOf(r) === s).length;
  ...
  return (
    ...
    >
      ...
      >
        <div>
          ...
          <div style={{ fontSize: 15, color: "#5C6B62" }}>
            {rows === null
              ? "Loading…"
              : `${merged.length} automation${merged.length === 1 ? "" : "s"} · ${countFor("Active")} active`}
          </div>
        </div>
        {newBtn}
      </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 13 }}>
                <span
                  style={{
                    ...
                    borderRadius: 10,
                    flex: "none",
                    background: r.invalid ? "rgba(224,121,107,.14)" : "rgba(54,215,237,.16)",
                    color: r.invalid ? "#C9543F" : "#1192A6",
                    display: "flex",
                    alignItems: "center",
                ...
                <span
                  ...
                  style={{
                    ...
                    flex: "none",
                    cursor: r.invalid || !canManage ? "not-allowed" : "pointer",
                    opacity: r.invalid ? 0.4 : 1,
                  }}
                >
                  <span
                    style={{
                ...
                {r.trigger ? (
                  ...
                  <span
                    style={{
                      ...
                      fontWeight: 600,
                      color: "#C9543F",
                      background: "rgba(224,121,107,.14)",
                      borderRadius: 8,
                      padding: "5px 11px",
                    }}
                  >
        ...
        {rows !== null && filtered.length === 0 && (
          ...
          >
            ...
            <div style={{ fontSize: 13.5, color: "#9AA59E", marginBottom: 18 }}>
              {seg === "All"
                ? "Create your first rule to run work in the background."
                : "Try another tab or create a new automation."}
            </div>
            {newBtn}
          </div>
        )}
      ...
      {toast && (
        ...
        >
          <span style={{ color: toast.error ? "#E0796B" : "#7FE8A0" }}>
            {toast.error ? "✕" : "✓"}
          </span>
          {toast.text}
        </div>
