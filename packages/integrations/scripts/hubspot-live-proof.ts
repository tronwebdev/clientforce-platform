...
async function main(): Promise<void> {
  ...
  const ws = await owner.workspace.create({
    data: { agencyId: agency.id, name: "HubSpot proof", slug: suffix, settings: {} },
  });
  const agent = await owner.agent.create({
    data: { workspaceId: ws.id, name: "Closer", goal: "close_deals", guardrails: {} },
  });
  ...
  const receipts: Record<string, unknown> = {
    note: "INT W4 live HubSpot one-way push — real vendor, real rails; contact/deal created in HubSpot as the proof artifacts.",
  };
  ...
  try {
    ...
    const row = await connectHubspotFields(deps, {
      workspaceId: ws.id,
      fields: { apiToken: TOKEN, defaultPipeline: "default" },
    }).catch((err) =>
    ...
    pass(
      "1 connect",
      `token stored — ${portalLabel} (write-only key → portal may be best-effort; push validates)`,
    );
    ...
    const createClaim = await owner.integrationDelivery.findFirst({
      where: { workspaceId: ws.id, kind: "crm_deal" },
    });
    ...
    receipts.createLedger = createClaim
      ? { id: createClaim.id, status: createClaim.status, detail: createClaim.detail }
      : null;
    const createWhy = createClaim
      ? `crm_deal ledger ${createClaim.status}: ${JSON.stringify(createClaim.detail)}`
      : "no crm_deal ledger row (the rule never fired)";
    if (!dealId)
      fail("2 create deal", `no crmDealId stored — the push did not land a deal · ${createWhy}`);
    if (createClaim?.status !== "delivered")
      fail(
        "2 create deal",
        `ledger status = ${createClaim?.status ?? "none"} (expected delivered) · ${createWhy}`,
      );
    ...
    await evaluateEventForRules(engineDeps, { ...baseEvent, id: `${suffix}-create` });
    const afterRedeliver = await owner.enrollment.findUniqueOrThrow({
      where: { id: enrollment.id },
    });
    const dealAfter = (afterRedeliver.meta as { crmDealId?: string }).crmDealId;
    const claimCount = await owner.integrationDelivery.count({
      where: { workspaceId: ws.id, kind: "crm_deal" },
    });
    if (dealAfter !== dealId || claimCount !== 1)
      fail("3 idempotency", `deal ${dealAfter} (was ${dealId}) · crm_deal rows ${claimCount}`);
    pass(
      "3 idempotency",
      "redelivery deduped — ONE crm_deal ledger row, SAME deal id (no duplicate deal)",
    );
    ...
    const moveClaim = await owner.integrationDelivery.findFirst({
      where: { workspaceId: ws.id, kind: "crm_stage" },
    });
    receipts.moveLedger = moveClaim
      ? { id: moveClaim.id, status: moveClaim.status, detail: moveClaim.detail }
      : null;
    if (moveClaim?.status !== "delivered")
      fail(
        "4 update stage",
        `ledger status = ${moveClaim?.status ?? "none"} · ${moveClaim ? JSON.stringify(moveClaim.detail) : "no crm_stage row"}`,
      );
    pass(
      "4 update stage",
      `REAL deal ${dealId} → closedwon · IntegrationDelivery ${moveClaim.id} delivered`,
    );
