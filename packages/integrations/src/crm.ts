...
export interface CrmPushResult {
  ...
  dealId?: string;
  /** create_deal only: whether the deal→contact link landed. A write-only key
   *  can't create associations (HubSpot reads both records to validate), so the
   *  deal is delivered with `associated=false` — the link is deferred, not lost. */
  associated?: boolean;
...
export async function deliverCrm(
  ...
  try {
    let dealId: string;
    let associated: boolean | undefined;
    ...
    return {
      ...
      ...(associated !== undefined ? { associated } : {}),
      detail:
        input.op === "create_deal"
          ? `deal ${dealId} created${associated ? " + linked" : " (link deferred)"}`
          : `deal ${dealId} moved to ${input.stage}`,
    };
  } catch (err) {
