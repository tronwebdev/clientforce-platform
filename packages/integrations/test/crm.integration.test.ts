...
function statefulHub(opts?: { authFail?: boolean; accountInfo?: number; assocFail?: boolean }) {
  let nextC = 1;
  let nextD = 1;
...
describe.skipIf(!hasInfra)("one-way CRM push (INT W4)", () => {
  ...
  it("connect tolerates a WRITE-ONLY Service Key: /account-info 401 stores the (trimmed) token, status connected — the probe is never a gate", async () => {
    await owner.integration.deleteMany({ where: { workspaceId: ws } });
    const hub = statefulHub({ accountInfo: 401 }); // the valid write-only key can't read /account-info
    const row = await connectHubspotFields(deps(hub), {
      workspaceId: ws,
      fields: { apiToken: "  sk-writeonly-key  ", defaultPipeline: "default" },
    });
  ...
  it("create_deal: upsert + create + associate → delivered with the deal id; a redelivery dedupes (no second create)", async () => {
    ...
    expect(res.associated).toBe(true); // the stub's v4 PUT succeeds → the link lands
  ...
  it("best-effort link FALLBACK: a write-only key that can't associate STILL delivers the deal (unlinked, never orphaned)", async () => {
    await owner.integration.deleteMany({ where: { workspaceId: ws } });
    await connectHub();
    const hub = statefulHub({ assocFail: true }); // the association PUT 400s ("associations are invalid")
    const res = await deliverCrm(deps(hub), {
      workspaceId: ws,
      op: "create_deal",
      sourceEventId: `evt-${suffix}-noassoc#rule:r#a:0`,
      contact: { email: `ada-${suffix}@t.test` },
      dealname: "unlinked deal",
      stage: "qualifiedtobuy",
    });
    expect(res.delivered).toBe(true); // the deal delivers despite the link failure
    expect(res.dealId).toBeTruthy();
    expect(res.associated).toBe(false); // link deferred, honestly recorded
    expect(hub.createCount()).toBe(1); // created once — not orphaned, not retried
  });
