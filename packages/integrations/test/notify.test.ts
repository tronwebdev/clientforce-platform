...
describe("matchNotificationKind", () => {
  ...
  it("never matches integration or unrelated events (loop safety)", () => {
    expect(
      matchNotificationKind(
        event("integration.notified.v1", { provider: "slack", kind: "new_reply" }),
      ),
    ).toBeNull();
    expect(matchNotificationKind(event("email.sent.v1"))).toBeNull();
    expect(
      matchNotificationKind(event("lead.stage_changed.v1", { fromStage: "a", toStage: "b" })),
    ).toBeNull();
  });
...
describe("notificationText", () => {
  it("is deterministic, owner-readable, and degrades without a contact", () => {
    expect(
      notificationText("new_reply", { contact: "Ada Lovelace", intent: "objection_price" }),
    ).toBe("↩ New reply from Ada Lovelace — objection price");
    expect(notificationText("new_reply", {})).toBe("↩ New reply");
    expect(notificationText("meeting_booked", { contact: "Ada" })).toBe(
      "📅 Meeting booked with Ada",
    );
