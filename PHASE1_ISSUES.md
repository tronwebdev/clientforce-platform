...
> **Owner amendment 2026-07-03 (DEC-024/025, PR #26 — supersedes the scope above):**
> Evidence-cited fills only: every `BusinessContext` field carries chunk citations from READY
> sources; no citation → no fill → **gap** (never model priors). Required fields are
> goal-conditional per the owner-approved registry in `packages/core` (approved 2026-07-03 with
> four edits — availability Required only on Reactivate; no `sender_identity` field, signature is
> a send-time merge token and P1.5 fails a send whose sender lacks a from-name; `company_address`
> workspace-level Required for email goals, consumed by the CAN-SPAM footer; `lead_magnet`
> Recommended + `qualification_criteria` added on Generate leads). Two layers: workspace
> (Brand kit, `agentId` null) + agent overlay; planner reads merged (agent wins); the gap checker
> spans both — workspace-covered fields are "✓ Found in your docs", never re-asked. Gap actions:
> "Type it" (`typed`, persists, re-distills) · "✦ Let AI" (`ai_decides`, audited, Undo). Launch
> gated on all gaps resolved. Re-distill on source READY/removed or typed-answer change.
...
- [ ] Every filled field cites chunk ids from the evidence pack; invalid/uncited fills are dropped server-side (DEC-024).
- [ ] Gap report is goal-conditional per the approved registry; two-layer merge with "covered by your docs" semantics (DEC-025).
- [ ] Typed answers survive re-distill; `ai_decides` is audited and undoable; launch gating exposed via `launchReady`.
...
**Credential handling (important):** platform secrets (SendGrid parent key, Google/MS OAuth _app_
...
- **Action/tool registry** — email-send is the first entry in a general **action catalog** the agent can
  ...
  email adapter as _one implementation of a tool interface_, not a bespoke email-only path, so Phase 2+
...
- **Unified event awareness** — every action emits result events (sent/opened/clicked → later
  ...
  context. The agent's loop is _goal + knowledge → plan → act (via tools) → observe (events) → re-plan_;
