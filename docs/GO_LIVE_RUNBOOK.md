...
| S0 | **A2P 10DLC brand + campaign REGISTERED and approved** for the platform messaging service — the account currently has ZERO registrations (DEC-067 status probe), and unregistered US-bound traffic is carrier-blocked | **HUMAN-VERIFIED · BLOCKING** | Twilio console → Messaging → Regulatory Compliance: brand approved AND the platform service's campaign approved; the probe's section 3b prints both statuses |
| S2 | **Advanced Opt-Out ON** on the Messaging Service (rail 1 of the DEC-062 double rail) — re-verify the auto-reply AFTER S0 lands, with a genuine external US recipient (same-account on-net keyword traffic never drew a reply, DEC-067) | **HUMAN-VERIFIED** | Console-only config with NO API — a new/re-created Messaging Service comes back with it OFF and nothing in the pipeline can detect that. Check: Twilio console → Messaging → Services → service → Opt-Out Management → Advanced Opt-Out enabled. Evidence of a regression: STOP replies get no confirmation and `sms-inbound-probe` shows no `outbound-reply` record (see DEC-064 verification) |
